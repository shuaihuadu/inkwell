...
H1–H6 是单向流水线，但实际工程里下游一定会发现上游遗漏。本节定义"在哪个阶段发现什么问题，应该回退到哪个阶段"，避免出现两类反模式：① 出问题就重头再来；② 在当前阶段就地"擦屁股"绕过上游。
...
| 任意阶段                        | Reviewer 反复打回超过 [`io-contracts.md` 第 9 节](../agents/_shared/io-contracts.md) 上限 | 当前阶段起点 | 返回 `blocked`，把"已收敛点 / 仍未达成共识点"写入 `task-board.md` 等待人工裁决 |
