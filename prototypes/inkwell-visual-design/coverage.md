...
Skills 区段只从统一 Skill 管理目录中选择并绑定，不在 Agent 设计页提供上传或创建入口。
...
上下文策略按 MAF 的真实职责拆分：`InkwellChatHistoryProvider` 负责读取并全量持久化 Session 消息；可选的 `CompactionProvider` 负责调用模型前的输入规整。候选流水线依次展示工具结果压缩、摘要压缩、滑动窗口与 Token 截断，每项独立配置触发条件。`AgentSnapshot` 的可序列化策略 DTO 与 MAF 映射尚未锁定，因此当前只展示候选配置，不开放保存。
