...
- [`CONTRIBUTING.md`](../CONTRIBUTING.md) 是分支、Pull Request、发布与 hotfix 流程的唯一事实源；执行 Git 操作前必须阅读并遵循，不在本文件复制整套命令
- 首次修改文件前检查当前分支。常规代码、文档和配置变更必须位于从最新 `dev` 创建的 `feature/*`、`fix/*`、`refactor/*`、`docs/*` 或 `chore/*` 短生命周期分支；若当前位于 `main` 或 `dev`，先保留用户已有改动并切换到合适分支，再开始编辑
- 普通变更只允许通过 PR 合入 `dev`；`dev -> main` 仅用于发布，`hotfix/* -> main` 仅用于已发布版本的紧急修复。禁止直接向 `main` / `dev` commit、push 或 force push
- 除非用户明确要求，不自行执行 commit、push、创建或合并 PR。用户要求提交或发布时，仍须遵守保护分支和目标分支规则，不得临时关闭 ruleset、绕过 required checks 或使用管理员权限强行合并
- 功能 PR 默认 Squash merge；`dev -> main` 发布 PR 使用 Merge commit；hotfix 合入 `main` 后必须同步回 `dev`
