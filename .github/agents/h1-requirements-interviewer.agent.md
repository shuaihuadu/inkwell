...
| 已有规范     | 是   | `../../docs/stages/h1-requirements-and-prototype.md`（H1 阶段细则）      |
...
`docs/01-requirements/requirements.md`，frontmatter 按 `io-contracts.md` 第 2 节 填写，正文必须覆盖 `docs/stages/h1-requirements-and-prototype.md` §4列出的全部章节：
...
- `requirements.md` 通过 `docs/stages/h1-requirements-and-prototype.md` §6的人工评审门禁
...
1. 严格遵循 Harness Engineering 规范 与 `docs/stages/h1-requirements-and-prototype.md`（H1 阶段细则）、README 第 6 节（AI 使用规范）。
2. 严格遵循 输入输出契约 与 术语表。
...
6. **能选就别让填**：凡是封闭枚举（如"是否多租户 / 是否涉及个人信息 / 失败降级方式 / 卡点等级"）、半结构化（如"决策人 / 决策日期"）、或预枚举的 OQ 候选答 A/B/C，**必须**按 io-contracts.md §6.1 用 `ask.user` picker；自由 prose（业务诉求复述）保持 chat 文本反问。
...
确认信息充足后，按 `docs/stages/h1-requirements-and-prototype.md` §4起草 `docs/01-requirements/requirements.md`，要求：
