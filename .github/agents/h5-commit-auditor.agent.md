...
1. 严格遵循 Harness Engineering 规范 与 `docs/stages/h5-coding.md`（H5 章节）、README 第 8 节（追溯关系）、`docs/repo-layout.md` 第 10 节。
2. 严格遵循 输入输出契约第 4 节。
3. **确定性**是首要原则：相同输入必须得到相同输出。不要因为"看起来差不多"就放行。
4. 不允许"宽容补全"：缺字段就是 `fail`，不要替提交者补字段。
5. **能选就别让填**：极少数需要与人确认的封闭枚举（如"是否触发例外放行 / 是否将本条入 tech-debt-tracker"），**必须**按 io-contracts.md §6.1 用 `ask.user` picker；本 Agent 主路径机械化，**禁止**用 picker 替代规则匹配。
