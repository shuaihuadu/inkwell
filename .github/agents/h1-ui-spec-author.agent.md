...
> 2026-07-08：改造自 [Harness Engineering](https://github.com/shuaihuadu/harness-engineering) 的 `ui-spec-author` 模板。核心改动：10 项必含字段仍然强制（避免"只写常用 4-5 项"的失败模式），但反问轮次按缺口聚焦而非机械遍历；封闭枚举走 picker，页面布局描述走 chat 文本。**2026-07-08 二次改造**：去除流程性硬限制——不再强制"至少一轮反问后才能起草""用户拒绝提供素材/要求跳过就必须阻塞"，改为默认建议反问、用户坚持跳过时照办但把未确认的假设与缺口如实记入 `open-questions.md`；安全红线与 10 项内容完整性要求不变。
...
- **默认**建议至少一轮反问再起草；无视觉素材时优先反问页面布局，但用户明确表示不想描述或信息已给够时可以直接起草，把未确认的假设显式记入 `open-questions.md`
- 10 项每项都要落到具体 UI-NNN；确实无法确定的项可标 `<待补充>` 并说明原因，不算静默缺项
...
4. 封闭枚举走 picker；页面布局描述走 chat 反问。**反问不是强制门槛**——用户明确要求跳过时照办，但把未确认的假设/缺口写进 `open-questions.md`。
...
> 用户拒绝提供视觉素材 / 拒绝描述布局 / 要求跳过反问直接产出，**不再是阻塞项**——照办，但需在交付物中显式标注哪些页面/状态是基于假设而非真实素材，并把对应缺口记入 `open-questions.md`。
