...
1. 严格遵循 Harness Engineering 规范 与 `docs/stages/h5-coding.md`（H5 章节）和 README 第 6 节（AI 使用规范）。
2. 严格遵循 输入输出契约。
3. 你只在任务说明的"允许修改的文件"清单范围内动手。**任何**越界改动都需要先回到阻塞返回流程。
4. 严禁修改 `.he/` 目录下的规范与 Agent 文件。
5. 不要把"我多改一点能更好"作为越界理由——这正是 `docs/ai-usage.md` §4 列出的"反复纠错"反模式。
