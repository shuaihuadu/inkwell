...
> 设计依据：H2 是少量决定下游大量代价的阶段；按 `docs/stages/h2-architecture.md` §5，每条选型必须留下"选择 / 为什么 / 替代方案 / 放弃理由 / 维护影响 / 成本性能安全交付影响"六字段，否则后续 H3/H5 一旦撞墙将无法回溯到决策证据。
...
`docs/03-architecture/architecture.md`，frontmatter 字段齐全（`stage: H2`，`upstream` 引用相关 REQ 与既有 ADR）。正文必须覆盖 `docs/stages/h2-architecture.md` §4列出的全部章节：
...
`docs/03-architecture/tech-selection.md`，每个关键技术选择按 `docs/stages/h2-architecture.md` §5六字段输出：
...
- `architecture.md` 通过 `docs/stages/h2-architecture.md` §6人工评审门禁
...
1. 严格遵循 Harness Engineering 规范 与 `docs/stages/h2-architecture.md`（H2 章节）。
2. 严格遵循 输入输出契约 与 术语表。
...
7. **能选就别让填**：决策点反问的备选项打分（备选 A/B/C 选哪个）、`open-questions-arch.md` OQ 候选答、SLA 与量级的封闭枚举档位（如"并发档：< 10 / 10–100 / 100–1k / > 1k"），**必须**按 io-contracts.md §6.1 用 `ask.user` picker；自由 prose（设计理由 / 残余风险描述）保持 chat 文本反问。
...
1. `docs/03-architecture/architecture.md`——覆盖 `docs/stages/h2-architecture.md` §4全部章节
