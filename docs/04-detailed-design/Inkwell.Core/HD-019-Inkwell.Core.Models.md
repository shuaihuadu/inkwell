...
4. **已 reviewed 的 [HD-006 `AgentRunRequest.ModelId: string?`](../Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md#32-agentruntimeagentrunrequestcs) 注释原文**："对应后端配置文件中的模型标识符，非数据库主键"——与上述三点结论完全一致。
...
- **Q&A-A（真实产品 / 技术分歧，未决）**：[ui-spec.md §4.3.3](../../01-requirements/ui-spec.md)"其他厂商专有参数：由后端配置定义渲染（v1 仅渲染配置中显式声明的参数；未声明的不出现）"要求模型目录能声明"额外参数 schema"，但已 reviewed 的 [HD-006 `AgentModelParameters`](../Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md#36-agentruntimeagentmodelparameterscs) 是固定字段 DTO（仅 `Temperature`/`TopP`/`MaxTokens`），没有可扩展字段位。是否需要：(A) 现在就给 HD-006 发 errata，新增 `IReadOnlyDictionary<string, object>? AdditionalParameters`（或更强类型的 schema 描述），本 HD `ModelEntryOptions` 同步新增对应字段声明可选参数清单；(B) 本 HD 现状维持（不声明额外参数 schema），v1 UI 该区段暂不渲染任何厂商专有参数，留 v2 backlog；(C) 其他折中方案。**本 HD 未擅自决定，也未修改 HD-006**。
- **Q&A-B（真实产品 / 技术分歧，未决）**：[requirements.md §5.2](../../01-requirements/requirements.md)"模型路由策略（默认模型、降级顺序）"是否需要本 HD 在 `ModelEntryOptions`/`ModelCatalogOptions` 里声明静态"降级顺序"字段（如 `int? FallbackPriority` 或顶层 `DefaultModelId`），供未起草的 `Inkwell.Core.AgentRuntime` 在运行时读取并执行故障转移；或者这份路由策略完全是 `AgentRuntime` 自己的配置段，与本 HD 的"目录展示元数据"无关。本 HD 当前**不**包含任何路由 / 降级顺序字段，倾向于（因 `AgentRuntime` 未起草，无法核实）**不**在此处理，但这是真实的架构边界问题，非本 HD 可单方面裁定。
- **Q&A-C（技术边界判断，供参考确认）**：本 HD 判断模型连接信息（Azure OpenAI Endpoint / Deployment Name / API Key）不属于本 HD 的 `ModelCatalogOptions`，理由见 [§0](#0-范围核实模型是否为持久化实体req-005--req-006-归属边界)。该判断技术上自洽（避免密钥出现在任何可能被序列化返回给 UI 的 DTO 里），但因未起草的 `Inkwell.Core.AgentRuntime` 尚不存在，无法核实该模块是否已有相反假设（例如假设"从 Models 目录里能拿到完整连接信息"）。若未来起草 `AgentRuntime` 时发现假设冲突，需要回来给本 HD 发 errata。
