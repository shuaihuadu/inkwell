...
> **本 HD 是 H3 第一张业务命名空间（`Inkwell.Core.*`）详细设计**，也是 `Inkwell.Core.csproj`（[AGENTS.md §3.1](../../../AGENTS.md)）第一次出现真实文件。此前 HD-001 ~ HD-013（HD-013 未起草）全部落在端口层（`Inkwell.Abstractions`）与 EFCore Provider 家族（`providers/Persistence/Inkwell.Persistence.EFCore*`）。
>
> **范围核实结论（非臆造，逐条附证据）**：
>
...
> - **REQ-017（Admin 用户管理能力）在本 HD 范围**——账号列表、创建账号、重置密码、解锁、禁用与启用均操作 `User`，归 `Inkwell.Core.Auth`；撤销他人共享 Agent 归 `Inkwell.Core.Agents`。管理员不能主动锁定账号，`IsLocked` 只由连续密码验证失败达到安全阈值后自动设置（[UF-012](../../01-requirements/user-flow.md#uf-012-admin-管理账号状态) + [AC-067](../../01-requirements/acceptance-criteria.md)）。
...
> - **OQ-005 closed §A 的账号创建范围已被 2026-07-19 errata 替代**——仍不提供自助注册、自助找回密码、外部 IdP 或修改用户名；初始 Admin 由 Seed 创建，后续账号由 Admin 创建，`IAuthService` 提供管理员 `ResetPasswordAsync`，但不提供匿名或用户自助重置密码。
...
> - 失败登录审计范围 = **A. 仅记录成功登录 / 登出**（历史确认；审计功能已取消，且“不落计数器”已由 2026-07-19 自动锁定实现替代：登录与客户端解锁的密码失败都写入 `FailedUnlockAttempts`）
...
>
> **2026-07-19 errata（UI-009 用户管理范围扩展）**：Owner 更新用户管理与 App Shell 原型并明确要求实现后，本 HD 顶部“OQ-005 仅 SQL/脚本开通账号”、§1.2“不提供自助改密 / 管理员重置密码 / 主动会话失效”及仅管理员解封的旧范围被新原型取代。实现范围扩展为：管理员创建账号并一次性显示临时密码、禁用/启用账号、重置密码，当前用户修改自己的密码，以及锁定账号的管理员解锁；临时密码登录后必须先完成改密才能访问工作区。`IsSuper` 统一更名为 `IsAdmin`，数据库通过 rename migration 保留已有值；`User` 新增 `IsDisabled`、`MustChangePassword`、`SessionVersion`。`IsLocked` 只由安全策略自动设置，管理员不能主动锁定账号；启用只清除 `IsDisabled`，不修改密码且不等同于解锁。管理状态或密码重置递增 `SessionVersion` 使目标账号既有会话立即失效，当前用户主动改密仅保留发起改密的当前会话。Core 必须重复校验管理员身份并禁止禁用当前登录账号，不能只依赖 WebApi policy 或桌面 UI。该 errata 仅更新技术事实，不修改本文人工维护的 `status` / `reviewers`。
...
- 当前用户修改密码；临时密码用户完成改密前不得进入工作区
- Admin 创建账号、重置密码、解锁、禁用与启用账号（[REQ-017](../../01-requirements/requirements.md) + [UF-012](../../01-requirements/user-flow.md#uf-012-admin-管理账号状态)）
- Admin 查询全量账号列表（供 UI-009；Service 内部按 Repository 分页循环取尽）
...
| --------------- | --------------------------------------------------------------------------- |
| 业务 Model      | `Auth/User.cs`                                                              |
| 具名 Repository | `Persistence/IUserRepository.cs`                                            |
| 业务对外接口    | `Auth/IAuthService.cs`                                                      |
| 业务 DTO        | `Auth/AuthSession.cs` / `Auth/UserListItem.cs` / `Auth/IssuedCredential.cs` |
...
| DI 装配  | `AuthBuilderExtensions.cs`（`UseDefaultAuthService()`，风格对齐 [HD-001 §6.1](../Inkwell.Abstractions/HD-001-Inkwell.Abstractions-foundation.md#61-典型用法与-architecturemd-3-示例对齐) 单实现登记风格） |
...
- WebApi 层的 HTTP 端点 / 授权策略（`[Authorize(Policy="RequireAdmin")]` 等）——归 `Inkwell.WebApi`；当前实现同时在 Core 校验 `actorUserId` 对应账号的 `IsAdmin` 与可用状态
...
- 批量主动删除某用户的全部缓存 key——当前实现通过每次会话校验读取用户并比较 `SessionVersion`，账号锁定、禁用或版本不匹配时删除当前 token；无需维护用户到 token 的二级索引
- `InkwellSeeder`（[HD-009](../Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md)，已 reviewed）新增"seed 默认账号"逻辑——dev 环境（Testcontainers 起真实 SqlServer / Postgres 实例）下启动即有可登录账号是实际需要的能力，但 `InkwellSeeder.SeedAsync()` 的具体实现属于 HD-009 已锁定文件，本 HD 不越权直接改写已 reviewed 文档；留待后续对 HD-009 发起 errata（本 HD 仅在此记录待办，不阻塞当前提交）
...
| Q2  | UI-001 登录失败审计范围的历史决策已随审计日志功能取消而失效；当前登录与 UI-002 解锁的密码失败共用 `User.FailedUnlockAttempts`，任一路径达到阈值都会自动锁定账号 | 2026-07-19 实现事实       | [AuthService 核心流程](#41-authservice-核心流程)                                                                                                                                                                                                                                                            |
...
| Q6  | `User` Model 仅实现 `IHasTimestamps`，不实现 `IHasRowVersion` 或 `IHasOwner`                                                                                    | 当前实现事实              | User 没有 Owner 语义；当前 EF Entity 与数据库表均无 `RowVersion`，账号状态写入由 Service 事务边界负责                                                                                                                                                                                                       |
...
| Q8  | `IAuthService.ListAccountsAsync` 返回 `IReadOnlyList<UserListItem>`（API 不分页，Service 内部循环读取 Repository 分页）                                         | 作者判断（非 Owner 拍板） | [requirements.md §6](../../01-requirements/requirements.md) 软目标“用户量级 ~100”；`IUserRepository.ListUsers` 仍返回 `PagedResult<User>`，Service 循环取尽后映射                                                                                                                                           |
| Q9  | `UserListItem` 不含 `PasswordHash`                                                                                                                              | 作者判断（非 Owner 拍板） | 防止密码哈希通过 Service → WebApi → JSON 序列化链路泄露                                                                                                                                                                                                                                                     |
...
2. 现有先例 [HD-006 `IAgentRuntime`](../Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md) 为**单实现**端口（同 Auth 一样没有可切换 Provider），仍然落端口层——判定标准是"业务对外接口"这一类别本身，而非"是否有多实现 / 多消费者"。
...
src/core/Inkwell.Abstractions/
  ...
  Auth/                                  # 新增子目录（业务对外接口，非 6 大基础设施端口之一）
    IAuthService.cs                      # 顶层业务门面（11 方法）
    AuthSession.cs                       # record，登录 / 校验会话成功后的返回 DTO
    User.cs                              # 账号业务 Model
    UserListItem.cs                      # record，账号列表投影（不含 PasswordHash）
    IssuedCredential.cs                  # record，仅显示一次的临时凭据
...
**当前文件布局**：`Auth/` 7 个 `*.cs`，另有 `Persistence/IUserRepository.cs`；以仓库真实目录为准，不维护易漂移的全项目累计文件数。
...
src/core/Inkwell.Core/
  ...
  Auth/
    ...
    AuthBuilderExtensions.cs             # UseDefaultAuthService()，风格对齐 HD-001 §6.1 单实现登记风格
```
...
| 职责         | 登录、会话、当前用户改密及 Admin 账号管理的业务端口                                                                                                                                                                                                                                 |
| 对外接口     | 11 个方法：`LoginAsync` / `LogoutAsync` / `ValidateSessionAsync` / `VerifyPasswordForUnlockAsync` / `ChangePasswordAsync` / `CreateAccountAsync` / `UnlockAccountAsync` / `DisableAccountAsync` / `EnableAccountAsync` / `ResetPasswordAsync` / `ListAccountsAsync`；签名以源码为准 |
| 内部函数或类 | 接口本身；唯一实现 `Inkwell.Core.Auth.AuthService`（[§3.8](#38-inkwellcoreauthauthservicecs)）                                                                                                                                                                                      |
| 输入数据     | 登录与解锁密码、当前会话 token；管理操作显式传入 `actorUserId`，目标操作传入 `targetUserId`；建号额外传入 `username` 与 `isAdmin`                                                                                                                                                   |
| 输出数据     | 登录、会话校验和当前用户改密返回 `AuthSession`；创建账号和重置密码返回 `IssuedCredential`；列表返回 `IReadOnlyList<UserListItem>`；状态操作返回 `Task`                                                                                                                              |
| 依赖模块     | `Auth/AuthSession.cs` / `Auth/IssuedCredential.cs` / `Auth/UserListItem.cs`                                                                                                                                                                                                         |
| 错误处理     | 用户名或密码无效 → `ArgumentException`；认证失败或管理员校验失败 → `UnauthorizedAccessException`；锁定、禁用或不允许的状态转换 → `InvalidOperationException`；目标不存在 → `KeyNotFoundException`；底层存储和缓存异常原样上抛                                                       |
| 日志要求     | 密码、临时密码、Session Token 与 PasswordHash 禁止写入日志；管理员操作可记录 actor/target user id 与 outcome                                                                                                                                                                        |
| 测试要求     | 当前行为测试位于 `tests/Inkwell.Core.Tests/Auth/AuthServiceTests.cs` 与 `tests/Inkwell.WebApi.Tests/Controllers/AuthControllerTests.cs`；独立的 `IAuthServiceContractTests` 尚未实现，列为待补而非已存在覆盖                                                                        |
...
| 对外接口     | `public sealed record AuthSession(Guid UserId, string Username, bool IsAdmin, bool MustChangePassword, string SessionToken, DateTimeOffset ExpiresAt)`                            |
...
### 3.3 `Auth/UserListItem.cs` 与 `Auth/IssuedCredential.cs`

| 字段         | 内容                                                                                                                                                                                                                              |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文件路径     | `src/core/Inkwell.Abstractions/Auth/UserListItem.cs` / `IssuedCredential.cs`                                                                                                                                                      |
| 职责         | `UserListItem` 是 UI-009 列表投影且不含 `PasswordHash`；`IssuedCredential` 承载创建或重置后仅显示一次的临时密码                                                                                                                   |
| 对外接口     | `UserListItem(Guid UserId, string Username, bool IsAdmin, bool IsLocked, bool IsDisabled, DateTimeOffset? LastLoginTime, DateTimeOffset CreatedTime)`；`IssuedCredential(Guid UserId, string Username, string TemporaryPassword)` |
| 内部函数或类 | 两个类型均为只读 record class                                                                                                                                                                                                     |
| 输入数据     | 由 `AuthService` 从持久化用户或新签发临时密码构造                                                                                                                                                                                 |
| 输出数据     | `ListAccountsAsync` 返回 `UserListItem` 集合；`CreateAccountAsync` / `ResetPasswordAsync` 返回 `IssuedCredential`                                                                                                                 |
| 依赖模块     | System.*                                                                                                                                                                                                                          |
| 错误处理     | DTO 本身不定义业务异常                                                                                                                                                                                                            |
| 日志要求     | `TemporaryPassword` 与 `PasswordHash` 禁止进入日志或 trace                                                                                                                                                                        |
| 测试要求     | 当前通过 AuthService 行为测试间接覆盖；独立 DTO 契约测试尚未实现                                                                                                                                                                  |
...
| 字段         | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| ------------ | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文件路径     | `src/core/Inkwell.Abstractions/Auth/User.cs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 对外接口     | `public sealed record class User : IHasTimestamps { public required Guid Id { get; init; } public required string Username { get; init; } public required string PasswordHash { get; init; } public bool IsAdmin { get; init; } public bool IsLocked { get; init; } public bool IsDisabled { get; init; } public bool MustChangePassword { get; init; } public int SessionVersion { get; init; } public int FailedUnlockAttempts { get; init; } public DateTimeOffset? LastLoginTime { get; init; } public DateTimeOffset CreatedTime { get; init; } public DateTimeOffset UpdatedTime { get; init; } }` |
...
| 依赖模块     | `Persistence/Mixins/IHasTimestamps.cs`（HD-002）；当前不实现 `IHasRowVersion` 或 `IHasOwner`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
...
| 测试要求     | 当前由 `AuthServiceTests` 通过真实 Model 间接覆盖；独立 `UserTests` 尚未实现                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |

### 3.7 `Persistence/IUserRepository.cs`

| 字段         | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| ------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文件路径     | `src/core/Inkwell.Abstractions/Persistence/IUserRepository.cs`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 内部函数或类 | 接口本身；实现由 `providers/Persistence/Inkwell.Persistence.EFCore` 补充（[HD-009](../Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md) 已锁定 `EfCorePersistenceProvider` 是唯一 `IPersistenceProvider` 实现，具名 Repository 实现 + Entity + Mapping 需追加到该 shared base——本 HD 不改写已 reviewed 的 HD-009，仅在此声明契约缺口，留待后续 errata）                                                                                                                                                                         |
...
| 错误处理     | `GetUser`/`GetUserByUsername` 找不到 → `KeyNotFoundException`；`AddUser` 用户名唯一约束冲突 → `InvalidOperationException`；命令超时 → `TimeoutException`；当前 User 不使用 `IHasRowVersion` 并发令牌                                                                                                                                                                                                                                                                                                                                                |
...
| 测试要求     | 独立 `IUserRepositoryContractTests` 尚未实现；后续应纳入 `tests/Inkwell.Providers.Contract/` 跨 Provider 契约测试                                                                                                                                                                                                                                                                                                                                                                                                                                   |
...
| 职责         | `IAuthService` 唯一实现；编排密码校验 / 会话缓存读写 / 失败计数                                                                                                                                                   |
...
| 内部函数或类 | 构造函数注入 `IPersistenceProvider persistenceProvider`、`ICacheProvider cacheProvider`、`IOptions<AuthOptions> authOptions`、`TimeProvider clock`；11 个公开方法实现见 [§4.1 逻辑总览](#41-authservice-核心流程) |
| 输入数据     | 同 `IAuthService`（[§3.1](#31-authiauthservicecs)）                                                                                                                                                               |
| 输出数据     | 同 `IAuthService`                                                                                                                                                                                                 |
| 依赖模块     | `Inkwell.Abstractions.Auth.*` / `Inkwell.Abstractions.Persistence.*` / `IPersistenceProvider` / `ICacheProvider` / 内部 `PasswordHasher` 与 `SessionTokenGenerator`                                               |
| 错误处理     | 同 [§3.1](#31-authiauthservicecs) 表述；状态写入由 `ExecuteInTransactionAsync` 包裹，异常直接向上传播                                                                                                             |
| 日志要求     | 同 [§3.1](#31-authiauthservicecs)                                                                                                                                                                                 |
...
| 内部函数或类 | 算法实现采用 `Rfc2898DeriveBytes.Pbkdf2` 静态方法（.NET 6+ BCL 内置，零第三方包）；`Hash` 返回值必须是**自描述**的单一字符串（内含算法标识 + 参数 + 盐 + 摘要，如 `PasswordHasher<TUser>` 惯用的版本前缀模式），使 `User.PasswordHash` 列的 schema **不随算法选型变化**（[§3.6](#36-authusercs) `PasswordHash` 定义为 `string`，无额外盐 / 迭代次数列）                                                                |
...
| 日志要求     | **禁止**在任何日志路径输出明文密码或哈希值本身；`Verify` 失败不单独打日志                                                                                                                                                                                                                                                                                                                                              |
...
| 对外接口     | `internal sealed record SessionCacheEntry(Guid UserId, string Username, bool IsAdmin, bool MustChangePassword, int SessionVersion, DateTimeOffset IssuedAt)`                                                                                            |
...
| 职责         | `IAuthService` 的 DI 注册入口，风格对齐 [HD-001 §6.1](../Inkwell.Abstractions/HD-001-Inkwell.Abstractions-foundation.md#61-典型用法与-architecturemd-3-示例对齐) 单实现登记风格                                          |
...
- **`LoginAsync`**：`GetUserByUsername` 未命中时对固定哑哈希执行一次 PBKDF2 校验后统一抛 `UnauthorizedAccessException`，避免账号枚举计时侧信道；真实账号密码错误时递增 `FailedUnlockAttempts`，达到 `MaxFailedUnlockAttempts` 自动设置 `IsLocked=true`。密码正确后依次拒绝 `IsDisabled` 与 `IsLocked` 账号；成功则签发包含 `IsAdmin`、`MustChangePassword`、`SessionVersion` 的缓存会话，并更新 `LastLoginTime`、清零失败次数。
- **`LogoutAsync`**：`GetAsync<SessionCacheEntry>` 取出 `UserId` → `RemoveAsync` → 命中则返回 `true`；未命中直接返回 `false`（幂等）。
- **`ValidateSessionAsync`**：先读缓存会话，再通过 `IUserRepository.GetUser` 读取当前用户；账号锁定、禁用或 `SessionVersion` 与缓存不一致时删除当前 token 并抛 `UnauthorizedAccessException`。成功时返回当前用户的 `IsAdmin` / `MustChangePassword` 与缓存 token，不重新签发会话。
- **`VerifyPasswordForUnlockAsync`**（[UF-002](../../01-requirements/user-flow.md#uf-002-自动锁定与解锁)）：密码正确 → `FailedUnlockAttempts=0`；密码错误与登录失败共用 `RegisterFailedPasswordAttemptAsync`，计数 +1，达到阈值自动设置 `IsLocked=true`。
- **`ChangePasswordAsync`**：校验当前密码后更新哈希、清除 `MustChangePassword`、递增 `SessionVersion`；重写当前 token 的缓存条目以保留发起改密的当前会话，其他旧会话在下次校验时失效。
- **`CreateAccountAsync`**：Core 复核 actor 为可用 Admin；创建账号时生成一次性临时密码，设置 `MustChangePassword=true`，返回 `IssuedCredential`，临时密码不进入日志。
- **`UnlockAccountAsync`**（[UF-012](../../01-requirements/user-flow.md#uf-012-admin-管理账号状态) + [AC-067](../../01-requirements/acceptance-criteria.md)）：`IsLocked=false` + `FailedUnlockAttempts=0` 同一事务写回；Core 校验 `actorUserId` 对应账号的 `IsAdmin` 与可用状态。
- **`DisableAccountAsync` / `EnableAccountAsync`**：Core 复核 Admin，禁止禁用当前登录账号；两者只修改 `IsDisabled` 并递增 `SessionVersion`，不修改 `IsLocked`、失败次数或密码，因此启用不等于解锁。
- **`ResetPasswordAsync`**：Core 复核 Admin，生成新临时密码，设置 `MustChangePassword=true` 并递增 `SessionVersion`；返回仅显示一次的 `IssuedCredential`。
- **`ListAccountsAsync`**：Core 复核 Admin 后，从 `page=1` 开始以 `Pagination.MaxPageSize` 循环调用 `IUserRepository.ListUsers`，直到 `HasNextPage=false`，最终映射为 `IReadOnlyList<UserListItem>`；Service API 不暴露分页或锁定过滤参数。
...
- 列：`Id`（PK，`Guid` v7）/ `Username`（`string`，唯一索引，长度上限 100）/ `PasswordHash`（`string`）/ `IsAdmin`（`bool`，默认 `false`）/ `IsLocked`（`bool`，默认 `false`）/ `IsDisabled`（`bool`，默认 `false`）/ `MustChangePassword`（`bool`，默认 `false`）/ `SessionVersion`（`int`，默认 `0`）/ `FailedUnlockAttempts`（`int`，默认 `0`）/ `LastLoginTime`（`DateTimeOffset?`，可空）/ `CreatedTime`+`UpdatedTime`（`IHasTimestamps`）
- 索引：`Username` 唯一索引（登录查找 + 唯一性约束双重用途）；`IsLocked`、`IsDisabled` 非唯一索引
- Entity（`UserEntity`）+ Mapping（`UserMappingExtensions`）+ `UserRepository` 已落在 `providers/Persistence/Inkwell.Persistence.EFCore/{Entities,Mapping,Repositories}/`，SQL Server 与 PostgreSQL migration 均已包含用户管理字段及 `IsAdmin` 重命名
...
- ~~`InkwellSeeder` 的默认管理员账号 seed 逻辑同样需要 errata 追加到 HD-009~~ **已解决**（当前实现从 `PersistenceOptions.Seed.AdminPassword` 读取首次 Seed 密码，默认值为 `admin`；EFCore Provider 使用 BCL 按与 `PasswordHasher` 相同的参数和格式生成随机盐 PBKDF2 哈希，不引用 `Inkwell.Core.Auth`，因此不产生跨层依赖。配置仅在 `admin` 账号不存在时生效）
- `tests/Inkwell.Providers.Contract/` 尚缺 User Repository 的 SQL Server / PostgreSQL 跨 Provider 契约用例
