...
3. `AgentToolBinding` → [HD-006 `JsonDelegateAIFunction`](../Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md#37-agentruntimejsondelegateaifunctioncs--agenttoolcallrecordcs) 的**形态翻译**（`IToolBindingResolver.ResolveAsync`）——构造 `JsonDelegateAIFunction` 实例（2026-07-07 errata，替代原 `AgentToolDefinition` 组装），`invokeAsync` 构造参数委托体内部只做"合并静态绑定参数 + 转发给 `IToolExecutor`"这一层纯 BCL 逻辑，零 MAF 依赖
...
| 内部函数或类 | 接口本身；实现（`Inkwell.Core.Tools.ToolBindingResolver`）内部：(1) `bindings` 为空集合 → 直接返回 `Array.Empty<AIFunction>()`；(2) 对每个 `binding` 经 `IPersistenceProvider.GetRepository<IToolRepository>().GetTool(binding.ToolId)` 取目录元数据；(3) 经内部 `ToolExecutorRegistry` 查找对应 `IToolExecutor`；(4) 构造 `new JsonDelegateAIFunction(tool.Name, tool.Description, tool.ParametersJsonSchema, mergedInvokeDelegate)`（2026-07-07 errata，`mergedInvokeDelegate` 为 §3.10 `BuildInvokeDelegate` 产出的合并委托，替代原 `AgentToolDefinition` record 组装） |
...
| 内部函数或类 | 接口本身；`ToolId` 对应 `ToolDefinition.Id`，供 `ToolExecutorRegistry` 按 `Guid` 索引；未来落地的具体工具（如计算器）各自实现该接口，经 DI `AddSingleton<IToolExecutor, XxxToolExecutor>()`（多重注册，[HD-001](../Inkwell.Abstractions/HD-001-Inkwell.Abstractions-foundation.md) `IEnumerable<T>` 消费惯例；2026-07-07 errata：生命周期改为 `Singleton`，与 `ToolExecutorRegistry`（[§3.9](#39-inkwellcoretoolstoolexecutorregistrycs)，本身 `Singleton`）消费 `IEnumerable<IToolExecutor>` 匹配——若未来具体工具需要 `Scoped` 依赖，应在执行逻辑内部通过 `IServiceScopeFactory` 手动开 scope，而非直接把执行器本身注册为 `Scoped`，避免重蹈 [design-review-report.md §24 B-1](../design-review-report.md) 的 DI 生命周期不匹配问题） |
| 输入数据     | `string argumentsJson`（模型生成的调用参数 JSON 文本，与 [HD-006 `JsonDelegateAIFunction`](../Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md#37-agentruntimejsondelegateaifunctioncs--agenttoolcallrecordcs) 构造参数 `invokeAsync` 委托的 `argumentsJson` 入参同形态，2026-07-07 errata） / `CancellationToken`                                                                                          |
| 输出数据     | `Task<string>`（结果 JSON 文本）                                                                                                                                                                                                                                                                        |
| 依赖模块     | System.\*（BCL `Func<>`/`Task<>`）                                                                                                                                                                                                                                                                       |
| 错误处理     | 具体实现自行决定异常语义；异常由调用方（`Inkwell.Core.AgentRuntime`，经 `JsonDelegateAIFunction` 构造时传入的 `invokeAsync` 委托间接调用，2026-07-07 errata）捕获转换（[§1.4](#14-与-inkwellcoreagentruntime-的边界声明工具定义-vs-工具执行)），本接口自身不规定异常类型                                                                     |
...
| 职责         | `IToolBindingResolver` 唯一实现；`AgentToolBinding` → `AIFunction`（经构造 `JsonDelegateAIFunction`，2026-07-07 errata）的翻译 + 执行委托合并逻辑（详 §3.4 内部逻辑描述）                                                                                                                                                                                                                                                                                                                                                                                          |
| 对外接口     | `internal sealed class ToolBindingResolver : IToolBindingResolver { public ToolBindingResolver(IPersistenceProvider persistence, ToolExecutorRegistry executorRegistry); public Task<IReadOnlyList<AIFunction>> ResolveAsync(IReadOnlyList<AgentToolBinding> bindings, CancellationToken ct = default); }`                                                                                                                                                                                                                 |
...
| 输出数据     | 见 §3.4                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                |
| 依赖模块     | `Inkwell.Abstractions.{Persistence.IPersistenceProvider,Persistence.Tools.IToolRepository,Persistence.Agents.AgentToolBinding,AgentRuntime.JsonDelegateAIFunction,Tools.IToolBindingResolver}`（`AgentToolDefinition` → `JsonDelegateAIFunction`，2026-07-07 errata） / `Microsoft.Extensions.AI.AIFunction`（外部包，经 `Inkwell.Abstractions` 传递可用，非新增依赖） / `IToolExecutor.cs` / `ToolExecutorRegistry.cs`（同文件夹） / `System.Text.Json.Nodes`（BCL）                                                                                                                                                                                                                                          |
...
| 内部函数或类 | (1) 校验 `builder` 非 null；(2) `builder.Services.AddScoped<IToolCatalogService, ToolCatalogService>()` + `AddScoped<IToolBindingResolver, ToolBindingResolver>()` + `AddSingleton<ToolExecutorRegistry>()` + `AddSingleton<IToolExecutor, CurrentDateTimeToolExecutor>()`（v1 唯一内置工具，[§8 Q&A-C](#8-需要-owner-确认的问题) 已解决；2026-07-07 errata·B-1 修复：生命周期由 `AddScoped` 改 `AddSingleton`，与 `ToolExecutorRegistry` 消费 `IEnumerable<IToolExecutor>` 的 `Singleton` 生命周期匹配，`CurrentDateTimeToolExecutor` 只依赖 BCL `TimeProvider` 单例安全，[design-review-report.md §24 B-1](../design-review-report.md) 已处理）；(3) 注册 `IValidateOptions<ToolOptions>` + 绑定 `configure`；(4) 返回 `builder` |
