...
- 具体内置工具的执行逻辑实现（如计算器 / 网页搜索的真实 `IToolExecutor` 落地类）——[requirements.md](../../01-requirements/requirements.md) 未枚举 v1 具体工具清单，本 HD 只锁定 `IToolExecutor` 注册机制，不预先假造具体工具，详见 [§8 Q&A-C](#8-需要-owner-确认的问题)
...
| 内部函数或类 | (1) 校验 `builder` 非 null；(2) `builder.Services.AddScoped<IToolCatalogService, ToolCatalogService>()` + `AddScoped<IToolBindingResolver, ToolBindingResolver>()` + `AddSingleton<ToolExecutorRegistry>()`；(3) 注册 `IValidateOptions<ToolOptions>` + 绑定 `configure`；(4) 返回 `builder`；**不**在此注册任何 `IToolExecutor` 具体实现（v1 无内置工具，[§8 Q&A-C](#8-需要-owner-确认的问题)） |
...
| 测试要求     | `ToolsBuilderExtensionsTests.cs`：(1) 调用后 `IToolCatalogService`/`IToolBindingResolver`/`ToolExecutorRegistry` 可从 `IServiceProvider` 解析；(2) `configure` 回调生效；(3) `builder` 为 `null` 抛异常                  |
...
- **业务失败 / 预期错误**：`KeyNotFoundException`（工具不存在 / 无匹配 `IToolExecutor`）/ `ArgumentException`（`ValidateToolBindingAsync` 缺必填参数）/ `InvalidOperationException`（`AddTool` 唯一约束冲突、`ToolExecutorRegistry` 构造期重复注册）
...
> 以下均未拍板，本 HD 默认实现按各自"本 HD 默认设计"处理，均为可回滚的最小假设，不代表 Owner 已认可。

### Q&A-A：Tool 目录是否需要运行期管理 API（Admin CRUD）
...
- 本 HD 暂按 A 实现，**不代答**，请 Owner 确认是否符合真实预期。

### Q&A-B：静态绑定参数与模型运行时参数的合并优先级
...
- 本 HD 暂按 A 实现，**不代答**，该决策涉及安全语义（是否允许模型绕过 Agent 配置的固定参数），请 Owner 确认。

### Q&A-C：v1 是否需要至少一个真实可用的内置工具
...
- 本 HD 暂按 A 处理（不越权臆造具体工具清单），**不代答**，请 Owner 明确 v1 是否需要至少一个可演示的真实工具、若需要请给出清单。
...
- 切到 `h3-detailed-design-reviewer` Agent 对本 HD 跑机械化评审前，请 Owner 先对 [§8](#8-需要-owner-确认的问题) 三个开放问题给出真实决策——尤其 Q&A-A（若选 B，`IToolRepository`/`IToolCatalogService` 接口形态需要扩充，属于评审前置条件）。
