...
| `agent_session_states` | Inkwell.Conversations | HD-017 | [REQ-010 + NFR-005](../01-requirements/requirements.md)                                                                                                             |
...
>
> **2026-07-15 替代性 errata**：原 `agent_session` 把产品会话列表元数据、消息归属和 MAF `SessionState` 混在同一行，且 `agent_chat_message.SessionId` 无法区分产品聚合与运行时 Session。本节现拆为 `agent_conversations`、`agent_chat_messages`、`agent_session_states` 三表；旧表名仅作为本说明中的历史术语保留，不再是当前数据库契约。`AgentVersionId` 只在产品会话表保存，状态表不重复该外键。
>
...
- `ConversationId`：`Guid`，同时为主键与外键 → `agent_conversations.Id`，`ON DELETE CASCADE`，形成 `AgentConversation 1:0..1 AgentSessionState`
- `SerializedState`：由绑定版本的 `AIAgent` 序列化，非空；EF Core Mapping 使用对应 Provider 的原生 `jsonb` / `json` 列类型，不得进入日志
- `Revision`：`long`，从 1 严格递增
- `LastRunId`：`string?`，长度上限 64；该检查点声称覆盖的最后一个完整成功服务端 `ExecutionId`，用于与 `agent_conversation.LastCommittedRunId` 对齐
- `UpdatedTime`：`DateTimeOffset`
- `RowVersion`：`IHasRowVersion`，状态保存比较交换

**约束与索引**：主键本身保证每个 Conversation 最多一条状态。`AgentVersionId` 不在本表重复保存；每次创建、反序列化和保存状态都先通过 `ConversationId` 读取不可变 `agent_conversation.AgentVersionId`。首次 Run 懒建 Revision 1；后续更新必须匹配旧 `Revision` 与 `RowVersion`。清空 Conversation 时在同一事务删除本行；状态与最后提交消息不一致时可删除并由绑定版本 Agent 重建。

### 删除与清空

- 删除 `agent_conversations` 由数据库级联删除 `agent_chat_messages` 与 `agent_session_states`。
- 清空不删除 `agent_conversations`：Service 在同一事务删除该会话全部消息和状态行，把 `Title` 置空并刷新 `UpdatedTime`；会话的 `AgentVersionId` 保持不变。
