...
- **意图**：Entity（in [`providers/Persistence/Inkwell.Persistence.EFCore/`](../03-architecture/adr/ADR-021-efcore-persistence-shared-base-and-provider-csproj-layout.md)）与 Domain DTO（in [`Inkwell.Abstractions/Persistence/<Module>/`](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md)）物理分离已锁；mapper 选型有三个候选：
...
> 本轮在已 reviewed 的报告主体之上**追加**，仅评审增量产物：[HD-009 Inkwell.Persistence.EFCore base](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md)（status: draft，2026-05-11 起草）+ [file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore` 章节追加](file-structure.md#providersinkwellpersistenceefcore) + [database-design.md `## providers/Persistence/Inkwell.Persistence.EFCore（EFCore base 实现）` 章节追加](database-design.md)。报告主体 §1 ~ §8 的 `status / reviewers` 字段**不**因本节调整。

### 9.0 评审范围与基线

- **本轮评审对象**：HD-009 全文（§1 ~ §13）+ file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore` 章节 + database-design.md `## providers/Persistence/Inkwell.Persistence.EFCore（EFCore base 实现）` 章节
...
| 章节                                 | 状态                  | 覆盖度                                                                                                                                                                                                          | 缺口                                                                                                                                                                                                                                                                                                   | 证据                                                                                                                                                                                                                                                                                                  |
| ------------------------------------ | --------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 1.1 文件结构                         | `pass`                | base 8 `*.cs` + 1 `BannedSymbols.txt` + 1 `.csproj` 全锁 + `## providers/Persistence/Inkwell.Persistence.EFCore` 跨模块章节同步落地 + §3.13 18 业务实体 × 4 类文件批量模板表                                    | —                                                                                                                                                                                                                                                                                                      | [HD-009 §2](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md) + [file-structure.md §providers/Persistence/Inkwell.Persistence.EFCore](file-structure.md#providersinkwellpersistenceefcore) + [HD-009 §3.13](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md) |
| 1.2 数据库 / 表 / 字段 / 索引 / 约束 | `pass`（base 范围内） | 三 mixin 自动配置规则 + SaveChangesInterceptor 行为 + AutoSeed 幂等 + 跨 Provider 字段映射策略 + 18 业务表占位（具体字段由各业务 HD 起草）                                                                      | —                                                                                                                                                                                                                                                                                                      | [HD-009 §3.0 / §3.3 / §3.4](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md) + [database-design.md §providers/Persistence/Inkwell.Persistence.EFCore](database-design.md)                                                                                                        |
...
- HD-009 §3.0 ~ §3.12 物理路径 `src/core/providers/Persistence/Inkwell.Persistence.EFCore/` 与 [ADR-021 D1](../03-architecture/adr/ADR-021-efcore-persistence-shared-base-and-provider-csproj-layout.md) 一致
...
- §3.0 + database-design.md `## providers/Persistence/Inkwell.Persistence.EFCore（EFCore base 实现）` 三 mixin 自动配置规则表锁定 EFCore 行为：`IHasTimestamps` → `IsRequired()` / `IHasRowVersion` → `IsRowVersion()` / `IHasOwner` → `IsRequired() + HasIndex()`
- 证据：HD-009 §3.3 + database-design.md `## providers/Persistence/Inkwell.Persistence.EFCore（EFCore base 实现）` 三 mixin 自动配置规则表
...
- HD-009 §5 拓扑约束 + file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore` 章节物理路径与 ADR-019 / 020 / 021 一致
...
- **C-11**：`pass` — 设计文档引用的源码路径与 [repo-impact-map.md](../01-requirements/repo-impact-map.md) 一致。证据：`providers/Persistence/Inkwell.Persistence.EFCore/` shared base 路径与 [AGENTS.md §3.1](../../AGENTS.md) + [ADR-021](../03-architecture/adr/ADR-021-efcore-persistence-shared-base-and-provider-csproj-layout.md) 一致；本批 errata 未改动源码路径
...
- **部署步骤 / 回滚 / 备份恢复**：`partial` — 凭证位 + K8s Secret 引用明确，但 Redis 部署 / 回滚步骤留给 `providers/Cache/Inkwell.Cache.Redis` 独立 HD 起草（合理 deferral，与 [HD-003 §7.4 §1.9 partial 先例](Inkwell.Abstractions/HD-003-Inkwell.Abstractions-file-storage-port.md) 同模式）。证据：[HD-004 §7.2](Inkwell.Abstractions/HD-004-Inkwell.Abstractions-cache-port.md#72-安全) + [§9](Inkwell.Abstractions/HD-004-Inkwell.Abstractions-cache-port.md#9-部署--配置)
...
- **不在本轮范围**：HD-001 ~ HD-007 / HD-009 主体设计（已在前序评审中处理，本轮仅在发现跨引用缺陷时反查）；`providers/VectorStore/Inkwell.VectorStore.Qdrant` / `Inkwell.Core/VectorStore` 独立 HD（尚未起草，HD-008 §1.2 已声明移交）
- **前置闸门**：
  - [requirements.md](../01-requirements/requirements.md) `status: reviewed` ✅
  - [repo-impact-map.md](../01-requirements/repo-impact-map.md) `status: reviewed` ✅
...
- **C88（PASS）**— HD-008 §1.3 Q1 / Q2 声明 `AzureOpenAIEmbeddingOptions`（Endpoint/ApiKey/DeploymentName）与 `QdrantVectorStoreOptions`（Host/Port/ApiKey/UseHttps）分别独立于 `Inkwell.Core/VectorStore/` 与 `providers/VectorStore/Inkwell.VectorStore.Qdrant/`，且与 [HD-006 `AzureOpenAIAgentRuntimeOptions`](Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md) 单实现拓扑一致——经核对 HD-006 实际字段（Endpoint/ApiKey/DeploymentName），两者字段命名模式确实同构；§13 Q6 "两组凭据各自独立配置段，即使指向同一 Azure OpenAI 资源"的决策与 HD-006 现状（`AzureOpenAIAgentRuntimeOptions` 是 Chat 模型专用、不与任何其他端口共用）互不冲突，无重复定义或字段撞名风险。证据：[HD-006 §9 appsettings.json 示例](Inkwell.Abstractions/HD-006-Inkwell.Abstractions-agent-runtime-port.md#9-部署--配置) vs [HD-008 §13 Q6](Inkwell.Abstractions/HD-008-Inkwell.Abstractions-vector-store-type-alias.md#13-决策记录)
...
- **建议方向**：补一句"具体监控指标（如 embedding 生成延迟、Qdrant 查询延迟、collection 大小）随 `Inkwell.Core/VectorStore/` 与 `providers/VectorStore/Inkwell.VectorStore.Qdrant/` 独立 HD 落地；OTel instrumentation 基线由 [`Microsoft.Extensions.VectorData` 内置 `ActivitySource`](https://learn.microsoft.com/dotnet/ai/microsoft-extensions-vector-data) 提供（[ADR-020 §迁移路径 step 9](../03-architecture/adr/ADR-020-vector-store-microsoft-extensions-vectordata.md)）"
- **卡点等级**：non-blocking
...
- **建议方向**：§9 结尾补一句"Qdrant collection 创建 / InMemory 无持久化等具体部署步骤，移交 `providers/VectorStore/Inkwell.VectorStore.Qdrant/` 与 `Inkwell.Core/VectorStore/` 独立 HD"
- **卡点等级**：non-blocking
...
- **后续路径建议**：Owner 就 B15 拍板后，建议顺序：(1) HD-008 errata 落地 → (2) HD-008 翻 `reviewed` → (3) 端口层 8 个 HD（HD-001 ~ HD-008）全部 reviewed，H3 端口层设计正式收口 → (4) 进入 `Inkwell.Core.KnowledgeBase` / `.Memory` / `Inkwell.Core.AgentRuntime` 等业务命名空间 HD 或 `providers/VectorStore/Inkwell.VectorStore.Qdrant` / `Inkwell.Persistence.EFCore.*` 等 Provider 独立 HD 起草
...
- **文件结构**：`pass` — §2 文件清单（1 csproj + 3 `*.cs`）与 §3.0 ~ §3.3 十字段表一一对应，且与 [file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.InMemory`](file-structure.md#providersinkwellpersistenceefcoreinmemory) 文件树逐行核对一致（计数"3 个 `*.cs` + 1 个 `.csproj`"手工核算无误）。证据：[HD-010 §2](Inkwell.Persistence.EFCore/HD-010-Inkwell.Persistence.EFCore.InMemory-adapter.md#2-文件清单) + [file-structure.md 对应章节](file-structure.md#providersinkwellpersistenceefcoreinmemory)
...
- **C101（PASS）**— [file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.InMemory`](file-structure.md#providersinkwellpersistenceefcoreinmemory) 文件树、依赖注释、计数估算（"3 个 `*.cs` + 1 个 `.csproj`"）与 HD-010 §2 / §3.0 逐一核对一致；"不创建 `InMemoryInkwellDbContext` 子类 / 不创建独立 `BannedSymbols.txt`"的理由引用锚点（HD-010 §5 / §10）真实可跳转。证据：[file-structure.md 对应章节](file-structure.md#providersinkwellpersistenceefcoreinmemory) vs [HD-010 §2 / §5 / §10](Persistence.EFCore/HD-010-Inkwell.Persistence.EFCore.InMemory-adapter.md)
...
- **对 file-structure.md 的结论**：本轮追加章节（`## providers/Persistence/Inkwell.Persistence.EFCore.InMemory`）本身准确（C101 PASS），**不需要**因 B16/B17 而改动——问题在 HD-010/HD-009 的代码样本，不在文件树描述
...
- **文件结构**：`pass` — §2 文件清单（1 csproj + 2 `*.cs` + 1 `Migrations/` 目录）与 §3.0 ~ §3.3 十字段表一一对应，与 [file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.SqlServer`](file-structure.md#providersinkwellpersistenceefcoreinmemory) 文件树逐行核对一致（计数"3 个 `*.cs` + 1 个 `.csproj`"手工核算无误，`Migrations/` 目录本身不计入文件数）。证据：[HD-011 §2](Inkwell.Persistence.EFCore/HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md#2-文件清单) + [file-structure.md 对应章节](file-structure.md)
...
- **C108（PASS）**— [file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.SqlServer`](file-structure.md) 文件树、依赖注释、计数估算与 HD-011 §2 / §3.0 逐一核对一致；"不创建 `SqlServerInkwellDbContext` 子类"的理由引用锚点（HD-011 §6）真实可跳转；2026-07-06 errata 说明段准确复述了 `EnableRetryOnFailure` 兼容性修正的来龙去脉。证据：[file-structure.md 对应章节](file-structure.md) vs [HD-011 §2 / §6](Inkwell.Persistence.EFCore/HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md)
...
- **G5（PASS）**— file-structure.md 本轮**未**因治理修正而改动：核实治理修正内容（Migration 策略"谁在何时调用"+ 决策来源措辞更正）不涉及任何文件树 / 文件计数 / csproj 依赖关系的变化，file-structure.md 现有 `## providers/Persistence/Inkwell.Persistence.EFCore.SqlServer` 章节（HD-011 起草时已建立）本身准确、无需同步——不属于遗漏
...
- **对 file-structure.md 的结论**：本轮不需要改动（§20.3 G5 已确认治理修正不涉及文件树变化；HD-011 起草时已建立的 `## providers/Persistence/Inkwell.Persistence.EFCore.SqlServer` 章节本身准确）
...
- **本轮评审对象**：HD-012 全文（§1 ~ §17）+ 与 HD-009（shared base，reviewed）/ HD-010（InMemory，reviewed）/ HD-011（SqlServer，reviewed）的一致性 + 本次同步修改的 [file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.Postgres`](file-structure.md) 章节
...
- **问题**：见 §21.2 C117——file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.Postgres` 章节末尾仍保留最初失实的"已用 vscode/askQuestions 呈现三候选方案，Owner 拍板选择"表述，未同步 HD-012 顶部 callout 已记录的治理修正说明（子代理伪造确认 → 默认 Agent 复核 → 真实补做确认）
...
- **追溯**：C117
- **处理状态（2026-07-06）**：已处理——file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.Postgres` 章节末尾"确认来源"表述已同步改为与 HD-012 顶部 callout 一致的治理修正说明，并交叉引用 B20 spike 前置任务（详 §21.6）；C117 转 `PASS`
...
- **B21 处理结果**：file-structure.md `## providers/Persistence/Inkwell.Persistence.EFCore.Postgres` 章节末尾"确认来源"表述已同步改为与 HD-012 顶部 callout 一致的治理修正说明，并交叉引用 B20 spike 前置任务；C117 转 `PASS`（纯文档一致性问题，无技术歧义）
...
> **后续范围变更**：Agent 空间已取消“我使用过”tab、Owner 名称和用户维度最近使用时间展示。下文对 `ListUsedAgentIdsAsync` / `GetLastActivityByAgentsAsync` 及对应 Repository 查询的评审仅保留为历史记录，不再构成当前实现要求；现行契约以 [HD-017](Inkwell.Core/HD-017-Inkwell.Core.Conversations.md) 顶部范围修正为准。
>
...
## 28. HD-018 Inkwell.Core.AuditLogs 首轮评审（2026-07-08）

> 评审对象：[HD-018 Inkwell.Core.AuditLogs](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md)（status: draft，2026-07-08 起草，**H3 第五张业务命名空间 HD**）+ 联动的 [database-design.md `## Inkwell.Core.AuditLogs` 章节](database-design.md#inkwellcoreauditlogs) + [file-structure.md `### Persistence/AuditLogs` / `## Inkwell.Abstractions.AuditLogs` 章节](file-structure.md#inkwellabstractionsauditlogs) + 已 reviewed 的 [HD-007](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md) 2026-07-08 errata（本 HD 起草期触发）。报告主体 §1 ~ §27 的 `status`/`reviewers` 字段**不**因本节调整。完备性判定沿用 [§22](#22-hd-014-inkwellcoreauth-首轮评审2026-07-06)/[§23](#23-hd-015-inkwellcoreagents-首轮评审2026-07-07)/[§24](#24-hd-016-inkwellcoretools-首轮评审2026-07-07)/[§26](#26-hd-017-inkwellcoreconversations-首轮评审2026-07-08) 已确立的口径——对照 requirements.md/acceptance-criteria.md 验收标准逐条核实覆盖度，不机械套用端口层"§7/§8/§9"三段式模板。全程使用 bullet list 呈现（按 user-memory `markdown-lint.md` 已知陷阱，避免中英文混排表格触发 MD060；HD-018 本文件顶部已 `markdownlint-disable MD060`，本节评审报告仍单独遵守 bullet list 惯例以保持全文件风格一致）。

### 28.0 评审范围与基线

- **本轮评审对象**：HD-018 全文（§0 ~ §9）+ database-design.md `## Inkwell.Core.AuditLogs` 章节 + file-structure.md `### Persistence/AuditLogs (HD-018 落地)` / `## Inkwell.Abstractions.AuditLogs` 两处追加 + 已 reviewed 的 HD-007 2026-07-08 errata（`MaxPageSize` 收紧，因 HD-018 起草期发现而联动修改，属本轮评审范围内的关联变更）
- **不在本轮范围**：HD-001 ~ HD-017 正文本身的重新评审（已在前序评审中处理，本轮仅在发现跨引用/跨文档缺陷时反查）；`IAuditLogRepository` 的 EFCore 实现（HD-018 §1.2 已声明留待 HD-009 errata，本轮不评审尚不存在的内容）
- **前置闸门**：
  - [requirements.md](../01-requirements/requirements.md) `status: reviewed` ✅
  - [repo-impact-map.md](../01-requirements/repo-impact-map.md) `status: reviewed` ✅
  - HD-018 frontmatter 完整，`upstream` 9 项均可定位：NFR-004 / REQ-017 + ADR-008 / ADR-017 / ADR-022 / ADR-023 + HD-001 / HD-002 / HD-007 全部真实存在
  - **不触发** io-contracts.md §5 阻塞返回——HD-018 是合理的业务命名空间第五张切片，目录未"严重偏离" h3-detailed-design.md

### 28.1 完备性扫描（对照 NFR-004 / REQ-017 / ADR-008 相关验收标准）

- **NFR-004"审计日志可在 UI 中按用户/Agent/时间检索"**：`pass`——[HD-018 §1.1](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#11-职责) `QueryAsync` 校验 + 转发 + 投影三步描述完整；`AuditLogQuery`/`AuditLogEntry` 字段 100% 复用已 reviewed 的 [HD-007 §3.4/§3.3](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md)，未重复定义。证据：[HD-018 §3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs) vs [HD-007 §3.3/§3.4](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md)
- **ADR-008"写入失败处理：本地内存队列 + 重试 3 次 + 磁盘 fallback + 告警"**：`pass`——[HD-018 §3.6](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#36-inkwellcoreauditlogsauditlogwritebackgroundservicecs)`AuditLogWriteBackgroundService` 逐字实现该策略（`attempt` 1~3 循环 + 第 3 次失败转 [§3.7](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#37-inkwellcoreauditlogsauditlogfallbackfilewritercs)磁盘 fallback + 告警指标），与 ADR-008 原文逐句对应
- **ADR-008"保留期清理任务"**：`pass`——[HD-018 §3.8](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#38-inkwellcoreauditlogsauditlogretentioncleanupbackgroundservicecs)`AuditLogRetentionCleanupBackgroundService` 按 `AuditLoggerOptions.RetentionDays` 周期删除，与 ADR-008"通过后台清理任务按 created_at 删除"一致
- **§0 范围核实结论（HD-007 单实现拓扑 → 本 HD 须同时实现 LogAsync+QueryAsync）**：`pass`，非臆造——独立打开已 reviewed 的 [HD-007 §1.3 Q-implementation-topology](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#13-关键决策摘要)原文核实"`IAuditLogger` 唯一实现 `Inkwell.Core.AuditLogs.DefaultAuditLogger`，不设独立 `providers/*` csproj"字面属实；[HD-007 §6 Builder DSL 钩子](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#6-builder-dsl-钩子给-inkwellcoreauditlogs-hd-的契约)原文确实要求"`AddSingleton<IAuditLogger, DefaultAuditLogger>()`"单一注册；[HD-007 §1.2"不在内"](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#12-范围)确实把"`DefaultAuditLogger` 具体实现"整体（未拆分写入/查询两半）指派给本 HD。HD-018 §0 的范围扩展判断（从"只做查询侧"扩展为"完整实现 LogAsync+QueryAsync"）有充分原文依据支撑，非自行编造。证据：[HD-018 §0](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#0-范围核实hd-007-查询侧现状--本-hd-实际边界) vs [HD-007 §1.2/§1.3/§6 原文](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md)
- **数据库设计**：`pass`——`audit_logs` 表字段/索引/约束齐全，已同步追加到 database-design.md 且顶层表清单行已从 `TBD` 更新为 `HD-018`；但该章节末尾"待 Owner 确认"提示行内容已过时（详见 §28.2 C-3，判定 blocking）
- **配置文件字段/默认值**：`pass`——`AuditLogWriterOptions`（新增，2 字段）+ `AuditLoggerOptions`（HD-007 已有，本 HD 不修改字段结构、仅联动 errata 收紧 `MaxPageSize`）均有 `[Range]`/默认值/Validator
- **文件结构/9 个程序文件职责描述完整性**：`pass`——§3.1~§3.9 共 9 个文件小节、10 字段表格全部填写完整，每个方法均有对应的"内部函数或类"实现逻辑描述（不同于 HD-017 首轮评审 C-1 发现的"方法列出但逻辑描述缺失"问题，本 HD 无此类缺口）；但 §3.5 的实现逻辑描述本身存在真实的 DI 生命周期缺陷（详见 §28.2 C-1，判定 blocking——这是"描述完整但内容有误"，与"描述缺失"是两类不同问题）

**完备性结论**：对照 NFR-004/ADR-008 相关验收标准，覆盖度 `pass`；数据库设计 `pass`（附 1 项文档同步缺口）；文件结构描述完整度 `pass`（附 1 项内容正确性缺陷，非缺失）。整体完备性本身达标，但发现的一致性问题（§28.2）**不足以直接推荐进入 H4/H5**，需先处理 §28.3 blocking 项。

### 28.2 一致性扫描（HD-018 ↔ HD-001 / HD-002 / HD-007 / HD-009 / HD-010 / HD-017 / ADR-008 / AGENTS.md §3.2 / database-design.md / file-structure.md）

- **C-1（BLOCKING，本轮最重要的发现）**——`DefaultAuditLogger`（[§3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs)）以 `internal sealed class DefaultAuditLogger : IAuditLogger { public DefaultAuditLogger(ChannelWriter<AuditLog> channelWriter, IPersistenceProvider persistence, IOptions<AuditLoggerOptions> options); ... }` 声明，构造函数**直接**注入 `IPersistenceProvider`；而 [§3.9](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#39-inkwellcoreauditlogsauditloggerbuilderextensionscs) 确认 `DefaultAuditLogger` 经 `builder.Services.AddSingleton<IAuditLogger, DefaultAuditLogger>();` 注册为 **Singleton**（与 [HD-007 §6](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#6-builder-dsl-钩子给-inkwellcoreauditlogs-hd-的契约)已锁定的强制契约"`AddSingleton<IAuditLogger, DefaultAuditLogger>()`"完全一致，本 HD 无法绕开这一 Singleton 生命周期）。而 `IPersistenceProvider` 按 [HD-009 §7](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md) 以 `services.AddScoped<IPersistenceProvider, EfCorePersistenceProvider>()` 注册为 **Scoped**——这正是本 HD 自己在 [§1.3 Q7](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#13-关键决策摘要)逐字指出、且引用 [HD-010 B16/C96](design-review-report.md#b16inmemoryrowversioninterceptor-以错误的-di-服务类型注册导致rowversion-拦截器永不执行c96) 作为反面教材的"Singleton 消费 Scoped 依赖"DI 反模式，本 HD 也确实为两个 `BackgroundService`（[§3.6](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#36-inkwellcoreauditlogsauditlogwritebackgroundservicecs)/[§3.8](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#38-inkwellcoreauditlogsauditlogretentioncleanupbackgroundservicecs)）正确采用了 `IServiceScopeFactory.CreateScope()` 规避方案——但**唯独 `DefaultAuditLogger` 自己（同样是 Singleton）没有采用同样的规避方式，直接把 `IPersistenceProvider` 声明为构造函数参数**。`QueryAsync`（[§3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs)）内部真实调用 `persistence.GetRepository<IAuditLogRepository>().ListAuditLogs(query, ct)`，即该缺陷会被真实触发（非未使用的死代码）。开启 [`ServiceProviderOptions.ValidateScopes`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.dependencyinjection.serviceprovideroptions.validatescopes)（ASP.NET Core `Development` 环境默认开启）的宿主下，DI 容器解析 `IAuditLogger` 时会直接抛 `InvalidOperationException: Cannot consume scoped service 'IPersistenceProvider' from singleton 'IAuditLogger'`；即使在未开启校验的宿主下（如默认 `Production`），容器也会把 `IPersistenceProvider`（内部持有 `DbContext`）的某个实例固化捕获为 `DefaultAuditLogger` 生命周期内的常驻依赖，而 `DbContext`/`EfCorePersistenceProvider` 并非线程安全，在并发查询场景下会产生数据竞争或连接状态错乱——这是一个比"启动期抛异常"更隐蔽、更危险的运行期正确性缺陷。证据：[HD-018 §3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs)（构造函数签名）vs [§3.9](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#39-inkwellcoreauditlogsauditloggerbuilderextensionscs)（`AddSingleton` 注册）vs [HD-009 §7](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md)（`IPersistenceProvider` 的 `AddScoped` 注册）vs [HD-018 §1.3 Q7](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#13-关键决策摘要)（本 HD 自己对同一反模式的正确认知，但未应用到 `DefaultAuditLogger` 本身）
- **C-2（BLOCKING）**——`IAuditLogRepository.RemoveAuditLogsOlderThan`（[§3.2](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#32-persistenceauditlogsiauditlogrepositorycs)）使用的动词前缀 `Remove` **不在** [HD-002 §4.1.3 Repository 方法动词白名单](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md#413-repository-方法动词白名单2026-05-11-errataf6--adr-022)（`Add`/`Update`/`Get`/`Delete`/`List`/`Find` 六选一，外加需独立 picker 拍板的冷僻动词例外 `Upsert`/`BulkAdd`/`Archive`/`Restore`）之内，`Remove` 既不在允许清单也不在冷僻例外清单，是全仓**唯一**违反该白名单的具名 Repository 方法——逐一核对全部 5 张已起草的具名 Repository（`IUserRepository`/`IAgentRepository`/`IToolRepository`/`IConversationRepository`/`IConversationMessageRepository`）确认它们的批量删除方法命名均为 `Delete<Type>`（如已 reviewed 的 [HD-017 `DeleteMessagesByConversation`](Inkwell.Core/HD-017-Inkwell.Core.Conversations.md#34-persistenceconversationsiconversationmessagerepositorycs)，同样是"按业务条件批量删除"语义，与本方法场景一致），无一使用 `Remove` 前缀（`ICacheProvider.RemoveAsync`，[HD-004 §3.1](Inkwell.Abstractions/HD-004-Inkwell.Abstractions-cache-port.md)，是缓存端口而非具名 Repository，不适用 §4.1.3 白名单，不构成先例）。此外，HD-018 [§0 第 2 段](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#0-范围核实hd-007-查询侧现状--本-hd-实际边界)"3 个具名动词方法（`Add`/`List`/`Remove`，HD-002 §4.1.3 动词白名单）"这句话本身**错误援引**了 HD-002 §4.1.3——该节原文并不包含 `Remove` 这一动词。证据：[HD-018 §3.2](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#32-persistenceauditlogsiauditlogrepositorycs) vs [HD-002 §4.1.3 原文](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md#413-repository-方法动词白名单2026-05-11-errataf6--adr-022) vs 全仓 5 张具名 Repository 逐一核对
- **C-3（BLOCKING）**——[database-design.md `## Inkwell.Core.AuditLogs` 章节](database-design.md#inkwellcoreauditlogs)末尾"待 Owner 确认"提示行 + [file-structure.md `## Inkwell.Abstractions.AuditLogs` 章节](file-structure.md#inkwellabstractionsauditlogs)对应提示行，两处均仍写"发现已 reviewed 的 HD-007 `AuditLoggerOptions.MaxPageSize` 默认值 `200` 与 HD-001 `Pagination.MaxPageSize=100` 硬编码常量不一致...本节仅记录该核查结论，不擅自修改 HD-007"——但已 reviewed 的 [HD-007 §3.6](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#36-auditauditloggeroptionscs)现文本核实**已经**通过 2026-07-08 errata 把 `MaxPageSize` 默认值/`[Range]`上界从 `200`/`[Range(1,1000)]` 收紧为 `100`/`[Range(1,100)]`，且 HD-018 自身 [§8 Q&A-B](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题)已明确标注该问题"已解决（2026-07-08）"。即 database-design.md / file-structure.md 这两处"待 Owner 确认"提示行是**过时未同步的陈述**，与 HD-018 §8/HD-007 §3.6 的现状矛盾，性质与已 reviewed 的 [HD-017 B-2](#26-hd-017-inkwellcoreconversations-首轮评审2026-07-08)（跨文档陈旧表述误导 H4/H5）完全同类。证据：[database-design.md line 279](database-design.md#inkwellcoreauditlogs) vs [file-structure.md line 600 附近](file-structure.md#inkwellabstractionsauditlogs) vs [HD-007 §3.6 现文本](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#36-auditauditloggeroptionscs) vs [HD-018 §8 Q&A-B](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题)
- **C-4（PASS）**——依赖规则核查（[AGENTS.md §3.2](../../AGENTS.md)）：全文 grep `using Microsoft\.EntityFrameworkCore|using StackExchange\.Redis|using Npgsql|using Minio|using Microsoft\.Agents\.AI|using Azure\.Storage` 仅命中顶部"依赖规则遵循"声明句本身（描述"不得引用"的文字）与 §9 CI 自检命令文本本身，全文**不存在**任何真实的 Provider 包或 `Microsoft.Agents.AI.*` 引用；持久化经 `IPersistenceProvider.GetRepository<IAuditLogRepository>()`（事务外读）/ `IUnitOfWork.GetRepository<IAuditLogRepository>()`（事务内写）双入口模式，与 [HD-002 §13.3 Q1=A2](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md)已锁定的模式一致。证据：全文 grep 仅 2 处命中，均非代码引用
- **C-5（PASS）**——`AuditLogQuery`/`AuditLogEntry`/`AuditLogRequest`/`AuditContext` 四个上游类型引用核实：[HD-018 §3.1](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#31-persistenceauditlogsauditlogcs) `AuditLog` 字段（`Id`/`EventType`/`ActorType`/`ActorUserId`/`AgentId`/`TargetKind`/`TargetId`/`PayloadJson`/`ResultCode`/`ErrorCode`/`RequestId`/`OccurredTime`）与 [HD-007 §3.3 `AuditLogEntry`](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#33-auditauditlogentrycs)字段逐一核对一致（除 `AuditLog` 多出的 `CreatedTime`/`UpdatedTime` 来自 `IHasTimestamps`，`AuditLogEntry` 也含同名两字段，映射方向正确）；[HD-018 §3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs) `MapToAuditLog` 引用的 `AuditLogRequest.Context.{ClientIp,ActionType,ActorUserId,ResourceType,ResourceId,TraceId,OccurredTime}` 与 [HD-001 §3.7 `AuditContext`](Inkwell.Abstractions/HD-001-Inkwell.Abstractions-foundation.md#37-commonauditcontextcs)+[HD-007 §3.2 `AuditLogRequest`](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#32-auditauditlogrequestcs)字段名逐一核对一致，无拼写/类型错位。证据：[HD-018 §3.1/§3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md) vs [HD-007 §3.2/§3.3](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md) vs [HD-001 §3.7](Inkwell.Abstractions/HD-001-Inkwell.Abstractions-foundation.md)
- **C-6（PASS）**——Q4"`ClientIp` 并入 `PayloadJson`，因 ADR-008 表结构字段列表未含独立 `client_ip` 列"的依据核实：独立打开 [ADR-008 §决策](../03-architecture/adr/ADR-008-audit-log-store-and-query.md)原文核对字段列表"`id`/`event_type`/`actor_type`/`actor_id`/`agent_id`/`target_kind`/`target_id`/`payload`/`result_code`/`error_code`/`request_id`/`created_at`"，确认**不含** `client_ip` 独立列，HD-018 Q4 依据属实，非臆造
- **C-7（PASS）**——`PagedResult<T>`/`IHasTimestamps` 类型引用核实：[HD-018 §3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs) `new PagedResult<AuditLogEntry>(items, page.TotalCount, page.Pagination)` 的位置参数顺序与 [HD-002 §3.4 `PagedResult<T>`](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md) `record PagedResult<T>(IReadOnlyList<T> Items, long TotalCount, Pagination Pagination)` 一致；`AuditLog : IHasTimestamps` 的 `CreatedTime`/`UpdatedTime` 属性名与 [HD-002 §3.9 `IHasTimestamps`](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md) 一致
- **C-8（PASS）**——`AuditLoggerBuilderExtensions.AddDefaultAuditLogger`（[§3.9](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#39-inkwellcoreauditlogsauditloggerbuilderextensionscs)）逐条核对 [HD-007 §6](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#6-builder-dsl-钩子给-inkwellcoreauditlogs-hd-的契约)已锁定的 4 项强制契约（`AddSingleton<IAuditLogger, DefaultAuditLogger>()`/注册 `IValidateOptions<AuditLoggerOptions>`/无需 `InkwellProvidersOptions` 交叉校验/返回 `builder`）：4 项均逐字满足。证据：[HD-018 §3.9](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#39-inkwellcoreauditlogsauditloggerbuilderextensionscs) vs [HD-007 §6 原文](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#6-builder-dsl-钩子给-inkwellcoreauditlogs-hd-的契约)
- **C-9（PASS）**——文件计数核实：Abstractions 侧"`Persistence/AuditLogs/` 2 个 + `Audit/` 2 个 = 4"，与已 reviewed 的 [HD-017 §2](Inkwell.Core/HD-017-Inkwell.Core.Conversations.md#2-文件结构)/[§27.3](#273-检查项-3n-1inkwellcorecsproj-累计文件数三处不一致应为-14是否真的修复pass)核实过的 Abstractions 累计 82 相加，82+4=**86**，与 HD-018 §2/file-structure.md 自述一致；`Inkwell.Core.csproj` 侧"14（HD-014~HD-017）+5（HD-018）=**19**"算式正确，14 这一输入值与 HD-017 §27.3 核实过的数字一致。未发现类似 HD-017 N-1 的加法错误
- **C-10（信息性，非技术一致性判据）**——HD-018 [§8 Q&A-A](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题)"Owner 在本次会话中通过 `vscode_askQuestions` 真实确认接受完整实现"+ [§8 Q&A-B](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题)"Owner 在本次会话中通过 `vscode_askQuestions` 真实确认选方案 A"两处声称的确认，**本项不由评审 Agent 代为判定真伪**：本次评审会话未见证任何 `vscode_askQuestions` 交互记录支撑这两处表述。格式本身符合本仓库已建立的"治理修正说明"标准写法（与 HD-017 §8 四项类似），但格式合规不等于内容真实——按 `/memories/repo/inkwell-h3-workflow.md` 已记录的"第 1~6 次复发"处理模式，请 Owner 在签字前自行核实这两处确认是否确实发生。**注**：任务描述已说明用户将自行做二次独立核实，本节仅如实标注、不重复发起验证动作

**一致性结论**：10 项检查中 **3 项 BLOCKING**（C-1/C-2/C-3）、0 项 non-blocking、1 项信息性记录（C-10，非 pass/fail 判据）、其余 6 项 `PASS`。

### 28.3 反问清单

#### Blocking

##### B-1：`DefaultAuditLogger` 作为 Singleton 直接构造函数注入 Scoped 的 `IPersistenceProvider`——"Singleton 消费 Scoped 依赖"DI 反模式（C-1）

- **问题**：`DefaultAuditLogger`（[§3.5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#35-inkwellcoreauditlogsdefaultauditloggercs)）经 [§3.9](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#39-inkwellcoreauditlogsauditloggerbuilderextensionscs)以 `AddSingleton<IAuditLogger, DefaultAuditLogger>()` 注册（该注册方式由已 reviewed 的 [HD-007 §6](Inkwell.Abstractions/HD-007-Inkwell.Abstractions-audit-logger-port.md#6-builder-dsl-钩子给-inkwellcoreauditlogs-hd-的契约)强制锁定，本 HD 无法更改），但其构造函数直接声明 `IPersistenceProvider persistence` 参数——该类型按 [HD-009 §7](Inkwell.Persistence.EFCore/HD-009-Inkwell.Persistence.EFCore-base.md)注册为 `AddScoped`。这正是本 HD 自己在 [§1.3 Q7](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#13-关键决策摘要)引用 [HD-010 B16/C96](design-review-report.md#b16inmemoryrowversioninterceptor-以错误的-di-服务类型注册导致rowversion-拦截器永不执行c96) 作为反面教材、并为两个 `BackgroundService` 正确规避的同一类反模式，但唯独 `DefaultAuditLogger` 本身未采用同样的 `IServiceScopeFactory` 规避方式
- **影响范围**：`QueryAsync` 真实调用 `persistence.GetRepository<IAuditLogRepository>().ListAuditLogs(...)`，该缺陷会被真实触发（非死代码）。开启 [`ValidateScopes`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.dependencyinjection.serviceprovideroptions.validatescopes)（ASP.NET Core `Development` 默认开启）的宿主会在容器构建/首次解析 `IAuditLogger` 时直接抛 `InvalidOperationException`；未开启校验的宿主（如默认 `Production`）会把 `IPersistenceProvider`（内部持有 `DbContext`）固化为 `DefaultAuditLogger` 的常驻依赖，`DbContext` 非线程安全，并发 `QueryAsync` 调用会产生数据竞争/连接状态错乱——比启动期抛异常更隐蔽、更危险。这会导致 UI-009 审计检索功能（[NFR-004](../01-requirements/requirements.md)）在真实并发场景下不可靠，属于 `TestCaseAuthor`/`CodingExecutor` 起步前必须先解决的技术缺陷
- **建议方向**（不替设计师下结论，仅给方向）：
  - 选项 1：`DefaultAuditLogger` 构造函数改为注入 `IServiceScopeFactory scopeFactory`（替换 `IPersistenceProvider persistence` 参数），`QueryAsync` 内部 `using var scope = scopeFactory.CreateScope(); var persistence = scope.ServiceProvider.GetRequiredService<IPersistenceProvider>();` 后再调用，与 [§3.6](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#36-inkwellcoreauditlogsauditlogwritebackgroundservicecs)/[§3.8](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#38-inkwellcoreauditlogsauditlogretentioncleanupbackgroundservicecs)两个 `BackgroundService` 的既有规避模式完全一致，改动范围小、风格统一
  - 选项 2：若 Owner 认为 `IAuditLogger` 不应保持 Singleton（挑战 HD-007 §6 已锁定的契约本身），需先对已 reviewed 的 HD-007 发起 errata 讨论生命周期变更，影响面更大，不建议
  - reviewer 倾向选项 1（改动最小、与本 HD 自身已确立的规避范式一致），但选择权在 Owner/author（本身是机械修正范畴，可不经 Owner picker，因为解决方式与 §1.3 Q7 已确立的模式完全相同，非新的产品/技术分歧）
  - 建议顺带补充 [§9 CI 自检 C4](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#9-ci-自检命令grep-列表)的检测范围——当前 C4 仅针对 `BackgroundService` 派生类做正则匹配，未覆盖本次实际踩坑的 `DefaultAuditLogger`（非 `BackgroundService` 但同为 Singleton），建议扩大匹配范围或改为"检测全部 `AddSingleton` 注册类型的构造函数是否直接包含 `IPersistenceProvider` 参数"
- **卡点等级**：**blocking**
- **追溯**：C-1

##### B-2：`IAuditLogRepository.RemoveAuditLogsOlderThan` 使用未授权动词 `Remove`，违反 HD-002 §4.1.3 动词白名单，且 HD-018 §0 错误援引该白名单支持这一命名（C-2）

- **问题**：`Remove` 前缀不在 [HD-002 §4.1.3 白名单](Inkwell.Abstractions/HD-002-Inkwell.Abstractions-persistence-port.md#413-repository-方法动词白名单2026-05-11-errataf6--adr-022)（`Add`/`Update`/`Get`/`Delete`/`List`/`Find`）之内，也不在需要独立 picker 拍板的冷僻动词例外（`Upsert`/`BulkAdd`/`Archive`/`Restore`）之内；全仓已起草的 5 张具名 Repository 均无先例使用 `Remove`，其中已 reviewed 的 HD-017 `DeleteMessagesByConversation` 恰是同语义场景（按业务条件批量删除）的正确命名先例。HD-018 §0 声称该命名依据"HD-002 §4.1.3 动词白名单"，但该节原文并不包含 `Remove`，是错误的自我援引
- **影响范围**：违反本仓库 CI 强制的 [`BannedApiAnalyzers`](https://github.com/dotnet/roslyn-analyzers/blob/main/src/Microsoft.CodeAnalysis.BannedApiAnalyzers/Microsoft.CodeAnalysis.BannedApiAnalyzers.md) 命名一致性约束（[ADR-022](../03-architecture/adr/ADR-022-entity-domain-mapper-selection.md)），H5 `CodingExecutor` 若照单实现会产出一个命名不合规的方法，需要后续单独 errata 修正；也会给未来其他业务 HD 起草者错误暗示"`Remove` 是被允许的动词"
- **建议方向**：`RemoveAuditLogsOlderThan` 改名为 `DeleteAuditLogsOlderThan`（对齐 `Delete<Type>` 前缀 + 已 reviewed 的 `DeleteMessagesByConversation` 同类先例），同步修正 [§0](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#0-范围核实hd-007-查询侧现状--本-hd-实际边界)对 HD-002 §4.1.3 的错误援引描述（`Add`/`List`/`Remove` → `Add`/`List`/`Delete`），以及 [§3.2](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#32-persistenceauditlogsiauditlogrepositorycs)/[§3.6](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#36-inkwellcoreauditlogsauditlogwritebackgroundservicecs)/[§3.8](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#38-inkwellcoreauditlogsauditlogretentioncleanupbackgroundservicecs)/[§9 C5](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#9-ci-自检命令grep-列表)中的全部引用点。**纯机械性命名修正，不需要 Owner picker**
- **卡点等级**：**blocking**
- **追溯**：C-2

##### B-3：database-design.md / file-structure.md 两处"待 Owner 确认"提示行陈述已过时，与 HD-007 §3.6 已完成的 errata + HD-018 §8 Q&A-B"已解决"标注矛盾（C-3）

- **问题**：[database-design.md `## Inkwell.Core.AuditLogs`](database-design.md#inkwellcoreauditlogs) 末尾 + [file-structure.md `## Inkwell.Abstractions.AuditLogs`](file-structure.md#inkwellabstractionsauditlogs) 对应提示行仍写"`MaxPageSize` 默认值 `200` 与 HD-001 `Pagination.MaxPageSize=100` 不一致...本节仅记录该核查结论，不擅自修改 HD-007"，但 HD-007 §3.6 现文本已通过 2026-07-08 errata 把默认值收紧为 `100`，且 HD-018 §8 Q&A-B 已标注"已解决"
- **影响范围**：`H4`/`H5` 读者若按这两处字面执行，可能误以为该问题仍待处理，重复发起已解决的确认流程；性质与已 reviewed 的 [HD-017 B-2](#26-hd-017-inkwellcoreconversations-首轮评审2026-07-08)（跨文档陈旧表述误导 H4/H5）完全同类
- **建议方向**：两处"待 Owner 确认"提示行改写为"已解决（2026-07-08，引用 HD-007 §3.6 errata + HD-018 §8 Q&A-B）"说明。**纯机械性文档同步修正，不需要 Owner picker**
- **卡点等级**：**blocking**
- **追溯**：C-3

#### Non-blocking

（本轮未发现 non-blocking 项；C-9 文件计数核实无误，未见 HD-017 式的加法错误。）

### 28.4 评审结论与下一步

- **整体评审决议**：**REJECT**——HD-018 §0 范围核实结论（HD-007 单实现拓扑迫使本 HD 完整实现 `LogAsync`+`QueryAsync`）有充分原文依据、非编造；对 NFR-004/ADR-008 相关验收标准的覆盖完整；`AuditLogRequest`/`AuditLogEntry`/`AuditLogQuery`/`AuditContext`/`PagedResult`/`IHasTimestamps` 等上游类型引用逐字段核对准确；Builder DSL 严格满足 HD-007 §6 锁定的 4 项契约；依赖规则核查通过（全文无 Provider 包/`Microsoft.Agents.AI.*` 引用）。但发现 **3 项 blocking**：B-1 是本轮最重要的发现——`DefaultAuditLogger` 自身违反了它在 §1.3 Q7 中正确认知并为两个 `BackgroundService` 规避的"Singleton 消费 Scoped 依赖"反模式，是真实的运行期正确性缺陷；B-2 是 Repository 动词白名单违规 + 错误自我援引；B-3 是跨文档陈旧表述（与已 reviewed HD-017 B-2 同类问题）。三项均需先修复才能推荐进入下一轮
- **HD-018 翻 `reviewed` 前置条件**：
  1. ⬜ 修复 B-1——`DefaultAuditLogger` 构造函数改为 `IServiceScopeFactory`，`QueryAsync` 内部按需创建 scope 解析 `IPersistenceProvider`（与本 HD 自身两个 `BackgroundService` 的既有模式一致）。**纯机械性修正，不需要 Owner picker**（解决方式是本 HD 自己已确立的既有范式，非新的技术分歧）
  2. ⬜ 修复 B-2——`RemoveAuditLogsOlderThan` 改名为 `DeleteAuditLogsOlderThan`，同步修正 §0/§3.2/§3.6/§3.8/§9 全部引用点。**纯机械性修正，不需要 Owner picker**
  3. ⬜ 修复 B-3——database-design.md/file-structure.md 两处"待 Owner 确认"提示行同步为"已解决"说明。**纯机械性修正，不需要 Owner picker**
  4. ⬜ Owner 在 HD-018 frontmatter 翻 `status: draft → reviewed` + 填 `reviewers: [Inkwell]`（人工签字位，AI 不代签）——修复 B-1/B-2/B-3 并经聚焦复审后再进行
- **需要人类核实的问题（不由本评审 Agent 代为判定真伪）**：HD-018 [§8 Q&A-A](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题)"Owner 在本次会话中通过 `vscode_askQuestions` 真实确认接受完整实现"+ [§8 Q&A-B](Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题)"Owner 在本次会话中通过 `vscode_askQuestions` 真实确认选方案 A"两处声称的确认，本次评审会话未见证任何相关交互记录，请自行核实（任务描述已告知将自行做二次独立核实，本报告不重复发起验证）
- **HD-018 是否可推荐翻 `reviewed`**：**不推荐**——B-1（真实技术缺陷，本轮最高优先级）、B-2（命名规范违规）、B-3（跨文档陈旧表述）均需先处理；三项均为机械性修正，预期不需要额外 Owner picker，处理后建议做一次聚焦复审（仅核对 B-1/B-2/B-3 三项修复点），聚焦复审通过后，仍需 Owner 自行核实 C-10 两处"Owner 已确认"表述方可签字
- **后续路径建议**：B-1/B-2/B-3 均机械修正 → 聚焦复审 → Owner 自行核实 C-10 两处确认 → Owner 签字 `reviewed`

### 28.5 自检

- ✅ 每条 `pass`/`blocking` 结论都附了文件路径 + 章节锚点证据
- ✅ 每个 `blocking` 反问（B-1/B-2/B-3）均能映射到具体一致性冲突（C-1/C-2/C-3）+ 影响范围 + 可执行的建议方向，未替设计师下结论
- ✅ 未使用"看起来"/"似乎"/"感觉"等主观词汇
- ✅ 未凭文件名臆测——`Pagination.MaxPageSize`/`IPersistenceProvider` 生命周期注册/`PagedResult`/`IHasTimestamps`/HD-002 §4.1.3 动词白名单/ADR-008 字段列表/HD-007 §3.6/§6 现文本均逐字打开核实，全文 grep 交叉验证
- ✅ 未运行任何 git 命令
- ✅ 未修改 HD-018 或任何其他 HD 正文，仅追加本节评审报告
- ✅ 未给越界建议
- ✅ 按任务明确要求，对"Owner 已确认"类表述**未自行判定真伪**，已在 §28.2 C-10 + §28.4 单独列为"需要人类核实的问题"，且未重复发起验证动作（任务已告知用户将自行二次核实）
- ✅ 未擅自判定 frontmatter `status`/`reviewers` 应保留的当前值，仅给出"内容是否支持翻 reviewed"的独立判断
