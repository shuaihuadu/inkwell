...
>
> **2026-07-06 errata·第九轮（design-review-report.md §20 B18/C103，HD-011 首轮评审发现，Owner picker 选项 1）**：§3.5 `MigrationRunner` 从单一 `RunAsync()` 拆分为两个独立公共方法 `MigrateAsync(ct)`（仅负责 schema 初始化）+ `SeedAsync(ct)`（仅负责按 `AutoSeedOnStartup` 开关执行 Seed）——修复"SqlServer/Postgres 场景跳过 Migrate 但仍需可选执行 Seed"无可行调用路径的设计空白；`Inkwell.WebApi` / `Inkwell.Worker` 启动代码对 InMemory 场景依次调用 `MigrateAsync(ct)` + `SeedAsync(ct)`，对 SqlServer / Postgres 场景只调用 `SeedAsync(ct)`。详 §13.9。
>
> **2026-07-06 errata·第十轮（design-review-report.md §20 B19/C104 + N31/C107，HD-011 首轮评审发现，Owner picker 选项 1 / 顺手处理）**：§3.11 `AddEfCorePersistenceBase()` 补 `services.AddOptions<PersistenceOptions>().BindConfiguration("Inkwell:Persistence")`——修复 `Inkwell:Persistence:CommandTimeoutSeconds`（及未来任何共享字段）从未被绑定进独立可解析的 `IOptions<PersistenceOptions>`、导致 `appsettings.json` 配置被静默忽略的缺陷；同轮顺手补齐 §3.0 csproj `<InternalsVisibleTo>` 声明（InMemory/SqlServer/Postgres 三个 final adapter，此前从未声明可见性授权）。三个 final adapter 共享此次修复，无需各自重复处理。详 §13.10。
...
- **依赖模块**：
  ...
  - `<InternalsVisibleTo>` 声明（2026-07-06 errata·第十轮，详 §13.10；回应 design-review-report.md §20 N31/C107）：`<ItemGroup><InternalsVisibleTo Include="Inkwell.Persistence.EFCore.InMemory" /><InternalsVisibleTo Include="Inkwell.Persistence.EFCore.SqlServer" /><InternalsVisibleTo Include="Inkwell.Persistence.EFCore.Postgres" /></ItemGroup>`——三个 final adapter csproj 借此访问本 csproj 的 `internal` 类型（`AddEfCorePersistenceBase()` / `MigrationRunner` / `EfCorePersistenceProvider` 等）
...
- **职责**（2026-07-06 errata·第九轮修订，详 §13.9；回应 design-review-report.md §20 B18/C103）：
  - **InMemory 场景**（dev / unit test，不受本轮修订影响）：`Inkwell.WebApi` / `Inkwell.Worker` 启动时依次调用本类 `MigrateAsync(ct)`（包装 `EnsureCreatedAsync()`）+ `SeedAsync(ct)`（按 `AutoSeedOnStartup` 开关执行）
  - **SqlServer / Postgres 场景**（integration test / prod）：`Inkwell.WebApi` / `Inkwell.Worker` 启动代码**只调用** `SeedAsync(ct)`——不再调用 `MigrateAsync(ct)`（其内部触发的 `IDbContextInitializer.InitializeAsync` 在这两种 Provider 下即执行 schema 变更，已改由 CI/CD pipeline 独立步骤在部署前执行，详 §13.8）
- **对外接口**（2026-07-06 errata·第九轮：从单一 `RunAsync()` 拆分为两个独立公共方法，详 §13.9）：
...
- **测试要求**：
  - `MigrateAsync(ct)`：InMemory 走 EnsureCreated 路径（由 `Inkwell.WebApi` / `Inkwell.Worker` 启动集成测试覆盖）；SqlServer / Postgres 走 Migrate 路径（用 mock `IDbContextInitializer` 做单测，真实 Migrate 行为改由 CI/CD 侧调用方 / 独立命令行工具的测试覆盖，不再属于 WebApi/Worker 启动集成测试范围）；超时抛 `TimeoutException`；`ct` 预取消抛 `OperationCanceledException`
...
- **输出数据**：副作用（注册服务）
- **依赖模块**：`Microsoft.Extensions.DependencyInjection` / [`Microsoft.Extensions.Options.ConfigurationExtensions`](https://www.nuget.org/packages/Microsoft.Extensions.Options.ConfigurationExtensions/)（`AddOptions<T>().BindConfiguration()`，2026-07-06 errata·第十轮新增，详 §13.10） / `Inkwell.Abstractions/Persistence/PersistenceOptions.cs` / 本 csproj 内全部类型
...
| `MigrationTimeoutSeconds`（默认 300）      | `MigrationRunner.MigrateAsync`（详 §13.9）内部 `CancellationTokenSource` 超时                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
### 13.9 2026-07-06 errata·第九轮（design-review-report.md §20 B18/C103，HD-011 首轮评审发现，Owner picker 选项 1：拆分 `MigrationRunner.RunAsync()` 为 `MigrateAsync` / `SeedAsync`）

**背景**：HD-011 首轮评审（design-review-report.md §20）发现 §3.5（2026-07-06 errata·第八轮修订后）`MigrationRunner` 仍只有一个公共方法 `RunAsync()`，内部把「Migrate（或 EnsureCreated）」与「按开关 Seed」耦合在同一方法体内。§13.8 与 [HD-011 §8](HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md#8-migration-执行策略2026-07-06-errata由webapi-启动自动执行改为-cicd-独立步骤非本-hd-拍板) 均承诺「SqlServer/Postgres 场景 `InkwellSeeder.SeedAsync()` 仍在 `Inkwell.WebApi` 启动时运行」，但若启动代码为了遵守「不再自动执行 Migration」而不调 `RunAsync()`，Seed 也无从触发——两个承诺互相矛盾，且没有第三条可行的调用路径（B18/C103）。Owner 在 chat picker 中拍板**选项 1**：把 `MigrationRunner` 拆成两个独立公共方法 `MigrateAsync(ct)` / `SeedAsync(ct)`，`Inkwell.WebApi` / `Inkwell.Worker` 启动代码对 InMemory 场景依次调用二者，对 SqlServer / Postgres 场景只调用 `SeedAsync(ct)`。

**改动范围**：

- **§3.5 `MigrationRunner.cs`**：对外接口从单一 `public async Task RunAsync(CancellationToken ct = default)` 拆分为 `public async Task MigrateAsync(CancellationToken ct = default)`（仅负责 schema 初始化）+ `public async Task SeedAsync(CancellationToken ct = default)`（仅负责按开关执行 Seed）；职责 / 内部函数或类 / 错误处理 / 日志要求 / 测试要求均按两方法重新拆分描述（详见现行 §3.5 文本）
- **§4.3 BCL 对照表**：「Migration 失败」/「Migration 超时」两行抛出位置从 `MigrationRunner.RunAsync`（§3.5）改为 `MigrationRunner.MigrateAsync`（§3.5）
- **[HD-011 §8 / §9](HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md)**：同步修订为「SqlServer 启动代码只调 `SeedAsync(ct)`，不调 `MigrateAsync(ct)`」的准确描述；§3.3 依赖 `MigrationRunner.RunAsync` 的两处引用同步改为 `MigrationRunner.MigrateAsync`（HD-011 侧改动详见该 HD 自身章节）

**未变项**：`IDbContextInitializer`（§3.6）接口签名与实现方式不变；`InkwellSeeder`（§3.4）签名与实现不变；`MigrationRunner` 的构造器参数不变；frontmatter `status: reviewed` / `reviewers: [Inkwell]` 不变（本轮是拆分既有方法为两个入口的接口级修正，属可通过小范围 errata 修复的设计空白，非重新评估已定架构决策，参照 §13.6 ~ §13.8 先例处理方式）。

**下游联动**：[HD-011](HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md) §8 / §9 / §3.3 同步修订；HD-012（Postgres final adapter，待起草）起草时直接采用 `MigrateAsync` / `SeedAsync` 两段式调用约定，不再沿用单一 `RunAsync()` 的旧表述；`Inkwell.WebApi` / `Inkwell.Worker` 进程层 DI 装配 HD（未起草）需在启动代码中落地「InMemory 依次调二者、SqlServer/Postgres 只调 `SeedAsync`」的具体调用路径。

### 13.10 2026-07-06 errata·第十轮（design-review-report.md §20 B19/C104 + N31/C107，HD-011 首轮评审发现，Owner picker 选项 1：`AddOptions<PersistenceOptions>().BindConfiguration(...)` + 补 `InternalsVisibleTo` 声明）

**背景**：HD-011 首轮评审（design-review-report.md §20）核实全仓库（HD-001 `AddInkwell()` / `InkwellBuilder.Build()`、HD-002、本 HD §3.11）均没有任何一处代码把 `appsettings.json` 的 `Inkwell:Persistence` 段绑定进独立可解析的 `IOptions<PersistenceOptions>`——现有绑定链只把整个 `"Inkwell"` 段绑定进**根** `IOptions<InkwellOptions>`（`.Persistence` 是其嵌套属性，与独立注册的 `IOptions<PersistenceOptions>` 是两个不同的 DI 服务实例）。HD-011 是第一个真正读取 `IOptions<PersistenceOptions>.Value.CommandTimeoutSeconds` 的 final adapter，因而首次暴露「配置被静默忽略」这一缺陷（B19/C104）。同一轮评审还发现 §3.0 csproj 从未声明具体的 `<InternalsVisibleTo Include="..." />` 条目，是 HD-009 自身的既有文档空白（对 InMemory/SqlServer/Postgres 三个 final adapter 一视同仁地缺失，非 HD-011 独有；N31/C107，non-blocking，评审建议与 B19 合并一次提交）。Owner 在 chat picker 中拍板**选项 1**：在本 HD §3.11 `AddEfCorePersistenceBase()` 内补 `services.AddOptions<PersistenceOptions>().BindConfiguration("Inkwell:Persistence")`（三个 final adapter 共享，一次修复覆盖 InMemory/SqlServer/Postgres）。

**改动范围**：

- **§3.0 csproj 依赖模块**：补 `<InternalsVisibleTo>` 声明——`<ItemGroup><InternalsVisibleTo Include="Inkwell.Persistence.EFCore.InMemory" /><InternalsVisibleTo Include="Inkwell.Persistence.EFCore.SqlServer" /><InternalsVisibleTo Include="Inkwell.Persistence.EFCore.Postgres" /></ItemGroup>`（N31 修复）
- **§3.11 完整代码**：`AddEfCorePersistenceBase()` 方法体首行新增 `services.AddOptions<PersistenceOptions>().BindConfiguration("Inkwell:Persistence");`（B19 修复）；依赖模块行补 [`Microsoft.Extensions.Options.ConfigurationExtensions`](https://www.nuget.org/packages/Microsoft.Extensions.Options.ConfigurationExtensions/)（`BindConfiguration` 扩展所在包）
- **注册顺序核实结论**（回应「是否与 final adapter 自身 `Configure<PersistenceOptions>(...)` 互相覆盖」的疑问）：[.NET Options 模式](https://learn.microsoft.com/dotnet/core/extensions/options) 对同一 `TOptions` 的多个 `IConfigureOptions<TOptions>` 委托按**注册顺序**依次执行、全部作用于同一个选项实例——每个委托只覆盖它显式赋值的属性，不会重置其余属性为默认值或清空前面委托已设置的值；`PostConfigure` 委托始终在全部 `Configure` 委托之后执行，与注册顺序无关。`AddEfCorePersistenceBase()`（含本行新增的 `BindConfiguration`）在 [HD-011 §3.1 `UseSqlServer(...)`](HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md#31-dependencyinjectioninkwellpersistenceefcoresqlserverservicecollectionextensionscs) 方法体内**先于** `services.Configure<PersistenceOptions>(o => o.ConnectionString = connectionString)` 被调用——因此实际执行顺序为：① `BindConfiguration` 绑定 `Inkwell:Persistence` 配置段全部字段（含 `CommandTimeoutSeconds`，也可能含 `ConnectionString`）；② 显式 `Configure<PersistenceOptions>(o => o.ConnectionString = connectionString)` 只覆盖 `ConnectionString` 一个字段（实现 [HD-011 §5.1](HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md#51-连接字符串来源) 「单一来源」约定）；③ 若调用方传入可选 `configure` 委托，通过 `PostConfigure` 在最后执行。三步不互相覆盖、不冲突——`CommandTimeoutSeconds` 等非 `ConnectionString` 字段在步骤 ② / ③ 均不受影响，配置生效链条完整
- **[HD-011 §3.1](HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md) 测试要求**：补一条「appsettings.json 设置 `CommandTimeoutSeconds` 后 `IOptions<PersistenceOptions>.Value.CommandTimeoutSeconds` 生效」用例（详见该 HD 自身章节）
