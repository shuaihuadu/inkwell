...
| Inkwell.Conversations  | `AgentSessionDefinition`   | `AgentSessionEntity`     | `agent_session`      | REQ-006 + NFR-005 |
| Inkwell.Conversations  | `AgentChatMessage`         | `AgentChatMessageEntity` | `agent_chat_message` | REQ-006 + NFR-005 |
...
> 17 个业务实体 × 4 类文件 = 68 个文件；加上 §3.0 ~ §3.6、§3.11、§3.12 共 8 个 base / DI / CI 文件 = base csproj 总计 76 个 `*.cs` + 1 个 `BannedSymbols.txt` + 1 个 `.csproj`。
