...
> 由 [HD-014 §2.1 / §3](Inkwell.Core/HD-014-Inkwell.Core.Auth.md) 锁定。`IAuthService`、Auth DTO 与 `User` Model 位于 `Auth/`；`IUserRepository` 位于 `Persistence/`。当前项目使用统一 `Inkwell` 命名空间，物理目录仅表达职责分组。
>
...
src/core/Inkwell.Abstractions/
  Auth/                                   # 新增子目录（HD-014）
    IAuthService.cs                       # 顶层业务门面（11 方法：登录/会话/解锁验证/改密/建号/解锁/禁用/启用/重置密码/列表）
    AuthSession.cs                        # record，登录 / 会话校验成功返回 DTO
    User.cs                               # 账号业务 Model（IHasTimestamps）
  ...
  Persistence/
    IUserRepository.cs                    # User 具名 Repository
```

**当前 Auth 相关文件**：`Auth/` 7 个 `*.cs`，加 `Persistence/IUserRepository.cs`；以仓库真实目录为准，不再维护易漂移的全项目累计文件数。
...
> `Auth/User.cs` + `Persistence/IUserRepository.cs` 已落地；`UserEntity` / `UserMappingExtensions` / `UserRepository` 位于 `providers/Persistence/Inkwell.Persistence.EFCore/{Entities,Mapping,Repositories}/`，SQL Server 与 PostgreSQL 均有对应 migration。
...
providers/Persistence/Inkwell.Persistence.EFCore/
  ...
  Entities/                           # 集中的 EF Entity 类（~30 个）
    ...
    UserEntity.cs                      # Username / IsAdmin / IsLocked / IsDisabled / MustChangePassword / SessionVersion / FailedUnlockAttempts / LastLoginTime
    …
  Configurations/                     # IEntityTypeConfiguration<TEntity>（~30 个）
    AgentEntityConfiguration.cs
    ConversationEntityConfiguration.cs
    UserEntityConfiguration.cs        # users 表 / Username 唯一索引 / IsLocked 与 IsDisabled 索引
    …
