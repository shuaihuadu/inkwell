...
- **Q-retention-days-default**：`AuditLoggerOptions.RetentionDays` 默认 `180`（对齐 [requirements.md §8.3](../../01-requirements/requirements.md) "审计日志：至少保留 6 个月"硬性要求）——**与 [ADR-008 §决策](../../03-architecture/adr/ADR-008-audit-log-store-and-query.md) 字面"v1 默认保留 90 天"（90 < 180）存在真实冲突**，本 HD 遵循 requirements.md 的硬性约束（NFR-004 用"必须"措辞），需要后续对 ADR-008 / [tech-selection.md §重复段](../../03-architecture/tech-selection.md) 补一条 errata（详 [§11 待补事项](#11-待补--待评审)，本 HD 不越权直接修改 `docs/03-architecture/` 文档）
...
- **RetentionDays 默认值与 ADR-008 字面冲突**（[picker Q-retention-days-default=B](#13-关键决策摘要)）：本 HD 采用 `180` 天对齐 [requirements.md §8.3](../../01-requirements/requirements.md) 硬性要求，但 [ADR-008 §决策](../../03-architecture/adr/ADR-008-audit-log-store-and-query.md) 字面仍是"90 天"，[tech-selection.md §123](../../03-architecture/tech-selection.md) 同样重复"保留 90 天"。本 HD **不越权**直接修改 `docs/03-architecture/` 文档（[H3-DetailedDesignAuthor 禁止事项](../../../.github/agents/detailed-design-author/AGENT.md)）——需要 Owner 在 H2 `h2-architect-advisor` 模式下对 ADR-008 补一条 errata（90 → 180）+ 同步 `tech-selection.md` 该段字面量，本 HD 的 `AuditLoggerOptions.RetentionDays = 180` 默认值届时才与 H2 文档完全对齐；在 errata 落地前，本 HD 与 ADR-008 存在已知的、显式记录的数字分歧
