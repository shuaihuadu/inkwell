...
>
> **2026-07-08 errata（修复 `MaxPageSize` 与 HD-001 `Pagination.MaxPageSize` 不一致）**：[HD-018 `Inkwell.Core.AuditLogs`](../Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md) 起草期发现，`AuditLoggerOptions.MaxPageSize` 原默认值 `200`（`[Range(1, 1000)]`）与 [HD-001 `Pagination.MaxPageSize`](HD-001-Inkwell.Abstractions-foundation.md) 硬编码常量 `100` 不一致——任何 `Pagination` 实例的 `PageSize` 在构造期就不可能超过 100，`MaxPageSize` 在 101~1000 区间的配置值/校验分支实际不可达（非崩溃性 bug，但与 HD-014/HD-016/HD-017 三次踩过的"构造期直接抛异常"属同一根因类别）。Owner 在本次对话中通过 `vscode_askQuestions` 真实确认修复方向：把 `AuditLoggerOptions.MaxPageSize` 默认值 / `[Range]` 上界收紧到 `[1, 100]`，与 `Pagination.MaxPageSize` 对齐。已在 [§3.6](#36-auditauditloggeroptionscs)/[§3.7](#37-auditauditloggeroptionsvalidatorcs)/[§13](#13-决策记录) 同步修复。
...
- **Q-pagination-bounds**：`AuditLoggerOptions.DefaultPageSize = 50`（对齐 ADR-008 "分页 50 条 / 页"字面值）/ `MaxPageSize = 100`（[2026-07-08 errata](#36-auditauditloggeroptionscs) 从 200 收紧，与 [HD-001 `Pagination.MaxPageSize`](HD-001-Inkwell.Abstractions-foundation.md) 硬编码常量 100 对齐，避免 101~1000 区间死区）
...
| 对外接口     | `public sealed class AuditLoggerOptions { [Range(1, 3650)] public int RetentionDays { get; init; } = 180; [Range(1, 90)] public int MaxQueryTimeRangeDays { get; init; } = 7; [Range(1, 500)] public int DefaultPageSize { get; init; } = 50; [Range(1, 100)] public int MaxPageSize { get; init; } = 100; public bool EnableSensitiveDataLogging { get; init; } = false; }`（`MaxPageSize` 2026-07-08 errata 从 `[Range(1,1000)]`/默认 `200` 收紧为 `[Range(1,100)]`/默认 `100`）                                                                                                                                                                                                                                                                                                                                                                                                                                        |
| 内部函数或类 | DataAnnotations 校验；`RetentionDays` 默认 `180`（[picker Q-retention-days-default=B](#13-关键决策摘要)，对齐 requirements.md "至少 6 个月"，**与 ADR-008 字面 90 天冲突**，详 [§11](#11-待补--待评审)）；`MaxQueryTimeRangeDays` 默认 `7`（ADR-008 字面值）；`DefaultPageSize`/`MaxPageSize` 默认 `50`/`100`（ADR-008 "50 条/页" + [HD-001 `Pagination.MaxPageSize`](HD-001-Inkwell.Abstractions-foundation.md) 硬约束上限，2026-07-08 errata 前曾是 `200`）；Provider 特定字段（无——本端口无可切换 Provider，[§1.3 Q-implementation-topology](#13-关键决策摘要)）不适用 |
...
| 测试要求     | `AuditLoggerOptionsTests.cs`：默认值（180 / 7 / 50 / 100 / false）、`appsettings.json` 绑定、`[Range]` 边界（1 / 上限 / 越界，`MaxPageSize` 上限 2026-07-08 errata 后为 100）                                                                                                                                                                                                                                                                                                                               |
...
| 内部函数或类 | (1) `Validator.TryValidateObject` DataAnnotations；(2) 跨字段：`DefaultPageSize <= MaxPageSize`（默认值 50 / 100 落在合法范围内，`MaxPageSize` 上限 2026-07-08 errata 后为 100，与 [HD-001 `Pagination.MaxPageSize`](HD-001-Inkwell.Abstractions-foundation.md) 对齐）                               |
...
| 测试要求     | `AuditLoggerOptionsValidatorTests.cs`：(1) DataAnnotations 边界合格；(2) 跨字段：`DefaultPageSize = 150 / MaxPageSize = 100` 拒（`DefaultPageSize` 超过收紧后的 `[Range(1,500)]` 内部值但违反跨字段约束，用具体越界值验证）；(3) 默认值（180 / 7 / 50 / 100）通过           |
...
| Q-pagination-bounds      | A：`DefaultPageSize=50`，`MaxPageSize=200`（**已被 2026-07-08 errata 推翻**：`MaxPageSize` 收紧为 `100`，详 [§3.6](#36-auditauditloggeroptionscs)） | 2026-07-05  |
...
- **Q-pagination-bounds**：备选 B（`MaxPageSize=100`）2026-07-05 起草期未选——200 提供更宽松的 UI 侧一次性加载空间，且不违反 ADR-008 "50 条/页默认展示"的字面（`MaxPageSize` 只是防御性上限，不是默认展示条数）。**2026-07-08 errata 推翻此决策**：Owner 通过 `vscode_askQuestions` 真实确认，`MaxPageSize=200` 与 [HD-001 `Pagination.MaxPageSize`](HD-001-Inkwell.Abstractions-foundation.md) 硬编码常量 `100` 不一致，101~1000 区间的配置值/校验分支实际不可达（[HD-018 `Inkwell.Core.AuditLogs` §8 Q&A-B](../Inkwell.Core/HD-018-Inkwell.Core.AuditLogs.md#8-需要-owner-确认的问题) 发现）；改选原备选 B，收紧为 `MaxPageSize=100`，与 `Pagination` 约束对齐，消除死区。
