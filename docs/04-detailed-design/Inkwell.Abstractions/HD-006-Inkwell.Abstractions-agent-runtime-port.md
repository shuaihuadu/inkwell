...
>
> **2026-07-20 Session checkpoint 删除 errata**：Conversation 运行不再持久化 MAF `AgentSession`。`AgentSessionState`、`IAgentSessionStateRepository` 与 `SaveSessionStateAsync` 已删除；每轮创建新 Session，并由 `InkwellChatHistoryProvider` 从 `AgentChatMessage` 唯一事实源恢复历史。下方关于 `SerializedState`、Revision、SessionStore 和状态/消息双回调的描述均被本 errata 取代。
>
