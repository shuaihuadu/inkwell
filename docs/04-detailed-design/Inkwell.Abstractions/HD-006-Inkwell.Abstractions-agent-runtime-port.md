...
> **2026-07-17 `AgentToolCallRecord` 删除 errata**：该类型原计划作为已删除的 `AgentTurnResult.ToolCalls` 元素，当前没有创建点或消费点。工具调用请求与结果由 `ChatMessage` 中的 `FunctionCallContent` / `FunctionResultContent` 持久化，AG-UI 流式路径使用框架原生工具事件，不再维护重复的 Inkwell 回溯 DTO。下方所有 `AgentToolCallRecord` 描述均为旧 facade 的历史设计，不再代表当前代码契约。
>
> **2026-07-20 Session checkpoint 删除 errata（已被 2026-07-26 errata 撤销）**：Conversation 运行不再持久化 MAF `AgentSession`。`AgentSessionState`、`IAgentSessionStateRepository` 与 `SaveSessionStateAsync` 已删除；每轮创建新 Session，并由 `InkwellChatHistoryProvider` 从 `AgentChatMessage` 唯一事实源恢复历史。下方关于 `SerializedState`、Revision、SessionStore 和状态/消息双回调的描述均被本 errata 取代。
>
> **2026-07-26 Session checkpoint 恢复 errata（查找键与 Store 基类部分已被 2026-08-03 errata 取代）**：`AgentSessionState` Model 与 `IAgentSessionStateRepository` 重新引入，取代上一条 errata。仓储接口收敛为四个方法：`AddSessionState` / `FindSessionStateBySessionKey` / `UpdateSessionState` / `DeleteSessionState`，均以 `SessionKey`（`AgentConversation.SessionKey`）为查找键，**不**包含旧设计的 Revision / CAS / 租约 fencing / `LastRunId` 语义，也不存在 `SaveSessionStateAsync` 双回调。Core 侧实现为 `InkwellAgentSessionStateStore`（继承 MAF `AgentSessionStore`），保存为 upsert、读取未命中或反序列化失败时回退为新建 Session；跨轮历史仍由 `InkwellChatHistoryProvider` 承担，两者职责不重叠。
>
> **2026-08-03 会话标识统一 errata**：`SessionKey` 整体移除，取代上一条 errata 中关于查找键与 Store 基类的部分。当前契约为：一、`AgentSessionState` Model 以 `ConversationId`（`Guid`）取代 `SessionKey`；二、仓储方法为 `AddSessionState` / `FindSessionStateByConversation` / `UpdateSessionState` / `DeleteSessionState`，均以 `Guid conversationId` 为查找键；`IAgentConversationRepository.GetConversationBySessionKey` 已删除；三、`InkwellAgentSessionStateStore` **不再继承** MAF `AgentSessionStore`，是按具体类型注册的 Scoped 服务，公开 `GetSessionAsync(AIAgent, Guid, CancellationToken)` / `SaveSessionAsync(AIAgent, Guid, AgentSession, CancellationToken)` / `DeleteSessionStateAsync(Guid, CancellationToken)`，返回 `Task`；四、upsert、未命中回退新建 Session 与两者职责不重叠的结论保持不变。
>
> **2026-07-12 替代性 errata（MAF 原生 Agent Factory，2026-07-16 构建边界修正）**：本 HD 原 `IAgentRuntime` facade、Run DTO、流式事件 DTO、Mapper 与取消注册表设计已被实现阶段的新主干取代；下方旧章节保留为历史评审依据，不再代表当前代码契约。现行契约为 `IAgentFactory.BuildAsync(AgentVersion, AgentBuildOptions, CancellationToken) -> ValueTask<AIAgent>`：`AgentVersion` 持有不可变 `AgentSnapshot`，Snapshot 固化名称、Instructions、模型与参数、工具/Skill 绑定及聊天历史策略；`AgentBuildOptions` 只承载 `AgentChatHistoryOptions`、`AgentToolDefinition` 与 `AgentSkillDefinition`，不暴露 `ChatHistoryProvider` / `AIFunction` 等 MAF 运行时实例。具体聊天历史 Provider 与 Skill 上下文 Provider 由 `Inkwell.Core.AgentRuntime` 转换；`JsonDelegateAIFunction` 已删除，Tool Definition 暂不转换为可执行工具，最终接入方式等待 Agent Factory review 定稿。MAF `AIAgent` 是 Inkwell 的公开核心运行时类型，AG-UI / OpenAI Chat Completions / OpenAI Responses / OpenAI Conversations Hosting 与 Workflow 直接消费它，不再经过 Inkwell 自建消息/事件协议。模型 SDK 客户端创建仍由 `Inkwell.Core.AgentRuntime` 的 Factory 实现负责。
>
> **2026-07-16 运行配置删除 errata**：`AgentRunOptions` 及其 Validator 已删除，`InkwellOptions` 不再保留 `AgentRuntime` 配置槽位。模型选择和显式调用参数由 `AgentSnapshot.ModelOptions` 中的 `AgentModelOptions` 固化；未设置的模型参数使用底层 SDK 默认值。运行取消和超时通过调用链 `CancellationToken` 与宿主策略控制，不再维护一套无运行时消费者的 Inkwell 全局默认值。
>
...
>
> **2026-07-15 替代性 errata（Factory / SessionStore / ChatHistoryProvider 分工，2026-07-16 协议核验修正）**：本 HD 的 Factory 边界保持不变，但 2026-07-10 的 `DatabaseChatHistoryProvider + AgentSession.StateBag` 方案由 [HD-017 §0](../Inkwell.Core/HD-017-Inkwell.Core.Conversations.md#0-2026-07-15-当前契约替代下方冲突章节) 取代。当前分工如下：(1) REST 创建产品 `AgentConversation` 并锁定不可变 `AgentVersionId`；(2) WebApi endpoint filter 根据已认证 `OwnerUserId + AgentId + RunAgentInput.ThreadId` 调用业务 Service 授权并以服务端 `ExecutionId` 占用单 Run 租约，再覆盖标准 `RunAgentInput.forwardedProps.inkwell` 为可信运行上下文；(3) singleton `RoutingAgent` 从 MAF `ChatOptions` 中的同一 `RunAgentInput` 读取该显式上下文，按绑定版本调用 `IAgentFactory`，不创建产品会话、不读取当前发布指针；(4) MAF 从根容器解析的 keyed singleton `InkwellAgentSessionStore` 按 conversation key 和数据库绑定版本创建、序列化和反序列化 wrapper Session，以 `ConversationId` 加载/保存 `AgentSessionState`；(5) `InkwellChatHistoryProvider` 对账 SDK 发送的完整消息快照，只保存新增输入与回复。适配器均不得通过 `AsyncLocal` / `IHttpContextAccessor` 传递业务上下文，也不得只凭裸 `threadId` 授权。
>
> **2026-07-16 Session 路由暂缓 errata**：`RoutingAgentSession` 与 `RoutingAgentSessionState` 已删除；上段第 (3) 至 (5) 项关于 wrapper Session、SessionStore 和实际 Agent Session 恢复的方案暂停，等待后续单独讨论。当前 `RoutingAgent` 只作为无状态协议路由代理：MAF 抽象要求的 Session 方法返回空占位状态，每次 Run 仅解析 HTTP Route / Claims 并调用 Core `IAgentBuildService`，再以 `session: null` 调用返回的实际 Agent。该临时行为不定义最终会话连续性方案。
>
> **状态与历史不是同一持久化回调**：`AgentSessionState` 是可丢弃检查点，数据库完整 `ChatMessage` 是历史唯一真值。Store 保存 `SerializedState + Revision + LastRunId + RowVersion`；Provider 以 `(ConversationId, ExecutionId, RunMessageIndex)` 幂等提交经历史对账确认的新增 request + response，并同步更新 `AgentConversation.LastCommittedRunId`。停止或断线后的已生成内容也提交消息批次，但不保存 SessionState。所有写入再次校验有效租约；实现不得假定 MAF 会把两次回调包进同一事务。加载时只有两个标记均非空且 `LastRunId == LastCommittedRunId` 才反序列化，否则由绑定版本 Agent 新建 Session 并从数据库消息重建。`@ag-ui/client@0.0.57` 每轮发送当前完整消息快照，后端不得把历史重复落库。
>
> **2026-07-07 errata（消除与 MAF `AIFunction` 的重复类型）**：Owner 在本次对话中通过 `vscode_askQuestions` 真实确认选择“重构：直接产出 `Microsoft.Extensions.AI.AIFunction`（消除重复）”——`Inkwell.Core.Tools` 不再产出本 HD 原 `AgentToolDefinition` 自定义 DTO，而是直接产出 [`Microsoft.Extensions.AI.AIFunction`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.ai.aifunction)。`AgentToolDefinition` 类型已删除；[§3.7](#37-agentruntimejsondelegateaifunctioncs--agenttoolcallrecordcs) 新增 `JsonDelegateAIFunction`（继承 `AIFunction`）承担原委托包装职责，`AgentToolCallRecord` 不变。`AgentRunRequest.Tools` 字段类型从 `IReadOnlyList<AgentToolDefinition>?` 改为 `IReadOnlyList<AIFunction>?`（详 §3.2）。`Microsoft.Extensions.AI.Abstractions` 已在 [HD-008 2026-07-06 errata·B15](HD-008-Inkwell.Abstractions-vector-store-type-alias.md) 加入 `Inkwell.Abstractions.csproj` 依赖白名单 + `GlobalUsings.cs`（当时为 `IEmbeddingGenerator<,>`），与 `Microsoft.Agents.AI.*` 是不同命名空间，不受 [AGENTS.md §3.2](../../../AGENTS.md) MAF 隔离禁区约束，本次复用同一白名单条目，不新增依赖。**范围声明**：本次仅修改本 HD 与 [HD-015](../Inkwell.Core/HD-015-Inkwell.Core.Agents.md) 两处引用；`Inkwell.Core.Tools`（[HD-016](../Inkwell.Core/HD-016-Inkwell.Core.Tools.md)）`IToolBindingResolver.ResolveAsync` 返回类型是否同步改为 `IReadOnlyList<AIFunction>` **不在本次任务范围内**，留待后续单独 errata 处理——处理前，HD-015 §3.4 对该方法返回值类型的描述与本 HD `AgentRunRequest.Tools` 之间存在已知的跨 HD 类型不一致（详见 §11 新增条目）。
>
...
>
> **2026-07-08 errata（消费 HD-017，措辞精确化 + 补齐序列化缺口）**：Owner 在本次对话中通过 vscode_askQuestions 真实确认需要修复 HD-017 起草期发现的两处问题：(1) 措辞精确化——原描述容易被误读为 Inkwell.Conversations 直接调用 IAgentInvocationService/IAgentRuntime；实际 Inkwell.Conversations 只提供历史消息查询能力，真正在调用前组装 AgentRunRequest 并发起调用的是未起草的 Inkwell.WebApi，本次已同步修正相关措辞。(2) AgentMessageContentPart 补齐 JsonPolymorphic/JsonDerivedType 特性标注——原设计缺少多态序列化契约，HD-017 ConversationMessage.ContentJson 序列化 `IReadOnlyList<AgentMessageContentPart>` 时会在 H5 因缺少类型鉴别器而反序列化失败，本次已在 §3.5 补齐。
>
> **2026-07-10 errata（H5 落地：采用 MAF `ChatHistoryProvider` 惯用法，`Messages` 语义变更）**：H5 编码阶段实现 `IAgentRuntime` 时，Owner 决定改用 [MAF `ChatHistoryProvider`](../../../../../microsoft/agent-framework/dotnet/src/Microsoft.Agents.AI.Abstractions/ChatHistoryProvider.cs) 惯用法承接对话历史，而非本文档原设计的"调用方预先查询拼装"。新增 `Inkwell.Core.AgentRuntime.DatabaseChatHistoryProvider`（`ChatHistoryProvider` 子类）委托 `IAgentConversationService` 读写历史，通过 `AgentSession.StateBag` 携带 `ConversationId`；`AzureOpenAIAgentRuntime` 改为 `agent.CreateSessionAsync()` 建立真实 `AgentSession` 并传入 `RunAsync`/`RunStreamingAsync`（不再传 `session: null`）。**影响**：`AgentRunRequest.Messages` 语义从"本轮 + 历史消息"改为"仅本轮新增消息（通常为单条最新用户消息）"；`Inkwell.WebApi` 不再需要在调用前查询历史拼装，历史读取与新消息落库均由 `DatabaseChatHistoryProvider` 在 MAF 执行管线内自动完成。本次已就地修订 §1.2 / §3.2 对应措辞（不新增独立文档），[HD-017 §1.1 / §1.4](../Inkwell.Core/HD-017-Inkwell.Core.Conversations.md) 同步更新。
>
...
- `IAgentRuntime` facade（§3.1）：定义业务命名空间调用 Agent 执行引擎的统一入口；`RunAsync`（同步取完整响应）/ `RunStreamingAsync`（流式取增量事件）/ `CancelRunAsync`（显式中断在途 Run）3 个方法（[picker Q-facade-scope=A](#131-起草期-picker-决策2026-07-05)），覆盖 [REQ-004](../../01-requirements/requirements.md)（Instructions 生效）/ [REQ-005](../../01-requirements/requirements.md)（模型选择）/ [REQ-006](../../01-requirements/requirements.md)（模型参数）/ [REQ-007](../../01-requirements/requirements.md)（工具调用）/ [REQ-010](../../01-requirements/requirements.md)（多轮对话）/ [REQ-014](../../01-requirements/requirements.md)（trace 全链路）/ [REQ-016](../../01-requirements/requirements.md)（多模态输入）场景
- 请求 / 响应 DTO（§3.2 ~ §3.7）：`AgentRunRequest`（一次 Run 调用的全部已解析上下文）、`AgentTurnResult`（非流式最终结果）、`AgentChatMessage` + `AgentMessageContentPart` 封闭子类型族（`TextPart` / `ImagePart` / `DocumentPart`，[REQ-016](../../01-requirements/requirements.md) 多模态载体）、`AgentModelParameters`（[REQ-006](../../01-requirements/requirements.md) temperature / top_p / max_tokens）、`JsonDelegateAIFunction`（[REQ-007](../../01-requirements/requirements.md) 工具描述 + 同进程调用委托包装为可调用的 [`Microsoft.Extensions.AI.AIFunction`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.ai.aifunction)，2026-07-07 errata 替代原 `AgentToolDefinition`）、`AgentToolCallRecord`（非流式路径下的工具调用回溯）
...
- **不在内**：
  ...
  - 工具注册表本身（哪些工具存在、参数 schema 从哪里来）——留 `Inkwell.Core.Tools` 业务 HD；本 HD 仅定义 `JsonDelegateAIFunction` 桥接类型，把工具描述 + 同进程调用委托包装为可调用的 `Microsoft.Extensions.AI.AIFunction`（2026-07-07 errata，废弃原 `AgentToolDefinition` DTO）
  - `AgentDefinition` 业务 Model 与持久化（已在 [HD-002 §4.1.2](HD-002-Inkwell.Abstractions-persistence-port.md) 锁定）
  - `AgentSession` / 对话历史的持久化与重建策略（[`Inkwell.Conversations`](../Inkwell.Core/HD-017-Inkwell.Core.Conversations.md) 业务 HD 提供 `IAgentConversationService` 历史查询 / 消息落库能力，由 `Inkwell.Core.AgentRuntime.DatabaseChatHistoryProvider` 在 Run 执行期委托调用自动完成，`Inkwell.WebApi` 不再需要预先查询拼装，2026-07-10 errata H5 落地精确化措辞；本 HD 端口不反向依赖 `IPersistenceProvider`，[picker Q-run-request-shape=A](#131-起草期-picker-决策2026-07-05)）
...
- **Q-facade-scope**：`IAgentRuntime` 含 3 方法（`RunAsync` / `RunStreamingAsync` / `CancelRunAsync`），不引入端口层 `CreateSessionAsync` / `DeserializeSessionAsync`（会话生命周期留 `Inkwell.Core.AgentRuntime` 内部管理）
...
- **Q-perf-budget**：宽松档，仅 facade 开销（不含模型推理延迟）——`RunAsync` / `RunStreamingAsync` P50 < 50ms / P99 < 200ms；`CancelRunAsync` P50 < 20ms / P99 < 100ms
- **Q-otel**：OTel span 命名 `agentruntime.<verb>`（`run` / `run_streaming` / `cancel_run`）+ 6 个私有字段 `agentruntime.agent_id` / `agentruntime.run_id` / `agentruntime.conversation_id` / `agentruntime.model_id` / `agentruntime.tool_call_count` / `agentruntime.operation_outcome`
...
src/core/Inkwell.Abstractions/
  AgentRuntime/                              # （新增子目录）
    ...
    JsonDelegateAIFunction.cs                # sealed class : AIFunction，工具描述 + 同进程调用委托包装为可调用 AIFunction（REQ-007，2026-07-07 errata 替代 AgentToolDefinition.cs）
    AgentToolCallRecord.cs                   # record，非流式路径下的工具调用回溯（同文件共置，不变）
...
> **csproj 依赖白名单**（2026-07-07 errata 更新）：HD-006 仍仅 [HD-001 §2 锁定的](HD-001-Inkwell.Abstractions-foundation.md) `Microsoft.Extensions.{DependencyInjection,Configuration,Options,Logging}.Abstractions` + `Microsoft.Extensions.VectorData.Abstractions`（HD-008 起用）+ `Microsoft.Extensions.AI.Abstractions`（[HD-008 2026-07-06 errata·B15 已加入白名单](HD-008-Inkwell.Abstractions-vector-store-type-alias.md)，本 HD 复用同一条目引入 `AIFunction` / `AIFunctionArguments` 基类型，非新增依赖）+ `System.Text.Json`（BCL 内置，`JsonDelegateAIFunction.JsonSchema` / `AgentToolCallRecord` 序列化）。**严禁**因本 HD 引入 `Microsoft.Agents.AI.*` / `Microsoft.Agents.AI.Abstractions` / `Microsoft.Agents.AI.OpenAI` 等任何 MAF 包（违反 [ADR-017 §依赖规则第 3/4 条](../../03-architecture/adr/ADR-017-backend-module-topology-ports-and-adapters.md) + [RISK-001](../../03-architecture/risk-analysis.md) MAF 接触面收敛约束）。
...
| 对外接口     | `public interface IAgentRuntime { Task<AgentTurnResult> RunAsync(AgentRunRequest request, CancellationToken ct = default); IAsyncEnumerable<AgentRunEvent> RunStreamingAsync(AgentRunRequest request, [EnumeratorCancellation] CancellationToken ct = default); Task<bool> CancelRunAsync(string runId, CancellationToken ct = default); }`                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
| 输入数据     | `AgentRunRequest request`（`RunAsync` / `RunStreamingAsync`） / `string runId`（`CancelRunAsync`） / `CancellationToken ct`（全部方法）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 输出数据     | `Task<AgentTurnResult>`（`RunAsync`） / `IAsyncEnumerable<AgentRunEvent>`（`RunStreamingAsync`） / `Task<bool>`（`CancelRunAsync`，`true` = 找到并已请求中断，`false` = `runId` 未知或该 Run 已结束）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
...
| 日志要求     | 实现层在每个方法入口 / 出口写 OTel span，命名 `agentruntime.<verb>`（`run` / `run_streaming` / `cancel_run`）；6 个 Inkwell 私有字段（`agentruntime.agent_id` / `agentruntime.run_id` / `agentruntime.conversation_id` / `agentruntime.model_id` / `agentruntime.tool_call_count` / `agentruntime.operation_outcome`）+ 5 个 OTel 标准 `exception.*` 字段（详 §4.3）；`AgentChatMessage.Content` 的实际文本 / 图片数据**不得**进入任何 OTel 字段（详 §7.2 PII 提示）                                                                                                                                                                                                                                                                                         |
...
| 职责         | 一次 Run 调用的全部已解析上下文；未起草的 `Inkwell.WebApi` 负责在调用前查好 Agent 配置（`Inkwell.Core.Agents`）/ 工具注册表（`Inkwell.Core.Tools`）并组装本 DTO（2026-07-08 errata 精确化消费方措辞——业务命名空间自身只提供查询能力，不直接调用 `IAgentRuntime`）（[picker Q-run-request-shape=A](#13-关键决策摘要)），端口本身不反向依赖 `IPersistenceProvider`；对话历史不再由 `Inkwell.WebApi` 预先查询拼装，改由 `Inkwell.Core.AgentRuntime.DatabaseChatHistoryProvider` 在 Run 执行期自动读写（2026-07-10 errata H5 落地）                                                                                                                                                                                                                                                                                                                |
| 对外接口     | `public sealed record AgentRunRequest { public required string RunId { get; init; } public required Guid AgentId { get; init; } public Guid? ConversationId { get; init; } public required IReadOnlyList<AgentChatMessage> Messages { get; init; } public string? Instructions { get; init; } public string? ModelId { get; init; } public AgentModelParameters? ModelParameters { get; init; } public IReadOnlyList<AIFunction>? Tools { get; init; } }`（`Tools` 类型 2026-07-07 errata 从 `IReadOnlyList<AgentToolDefinition>?` 改为 `IReadOnlyList<Microsoft.Extensions.AI.AIFunction>?`，详 §3.7）                                                                                                                   |
| 内部函数或类 | 构造期校验 `RunId` 非空、`Messages` 非空集合（`Messages.Count == 0` 且 `Instructions` 为 `null` 时不合法——无输入无指令则无法产生响应）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 输入数据     | 由调用方（`Inkwell.WebApi`，2026-07-08 errata 精确化——非"业务命名空间"直接组装，业务命名空间只提供查询能力）组装：`RunId`（客户端生成或 `Inkwell.WebApi` 端点生成，[REQ-014](../../01-requirements/requirements.md) trace 关联键）/ `AgentId`（[HD-002 `AgentDefinition`](HD-002-Inkwell.Abstractions-persistence-port.md) 主键） / `ConversationId?`（`null` = 新对话，[REQ-010](../../01-requirements/requirements.md)） / `Messages`（**仅本轮新增消息**，通常为单条最新用户消息，**不含历史**；历史由 `Inkwell.Core.AgentRuntime.DatabaseChatHistoryProvider` 委托 [HD-017 `Inkwell.Core.Conversations`](../Inkwell.Core/HD-017-Inkwell.Core.Conversations.md) `IAgentConversationService` 自动拉取拼接，2026-07-10 errata H5 落地） / `Instructions?` / `ModelId?` / `ModelParameters?`（未提供则实现层套用 `AgentRuntimeOptions` 默认值，[REQ-006](../../01-requirements/requirements.md)"使用默认"） / `Tools?`（`Inkwell.Core.Tools` 解析勾选的工具，[REQ-007](../../01-requirements/requirements.md)，2026-07-07 errata 起为 `AIFunction` 实例集合） |
| 输出数据     | `AgentRunRequest` 实例                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| 依赖模块     | `AgentChatMessage.cs` / `AgentModelParameters.cs` / `Microsoft.Extensions.AI.AIFunction`（外部包类型，2026-07-07 errata 替代 `AgentToolDefinition.cs`）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| 错误处理     | `RunId` 为 null/empty → `ArgumentException`；`Messages` 为空集合且 `Instructions` 为 `null` → `ArgumentException`（构造期校验，非运行期异常）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                             |
| 日志要求     | DTO 自身不做日志；`RunAsync` / `RunStreamingAsync` 在 `agentruntime.run(_streaming)` span 输出 `agentruntime.agent_id` / `agentruntime.run_id` / `agentruntime.conversation_id`；`Messages` 内容不进 OTel（详 §7.2）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 职责         | `RunAsync`（非流式）返回的完整结果；携带最终响应消息 + 完整工具调用回溯，保证非流式路径与流式路径在调试 trace 中的可见性对等（[picker Q-tool-result-passthrough=A](#13-关键决策摘要)，[REQ-007](../../01-requirements/requirements.md) / [REQ-014](../../01-requirements/requirements.md) 验收要求）                                                 |
...
| 输入数据     | 由实现层构造（`RunAsync` 内部调用 MAF 完成后组装）                                                                                                                                                                                                                                                                                                   |
| 输出数据     | `AgentTurnResult` 实例                                                                                                                                                                                                                                                                                                                               |
| 依赖模块     | `AgentChatMessage.cs` / `AgentToolCallRecord.cs` / `AgentModelParameters.cs`                                                                                                                                                                                                                                                                         |
| 错误处理     | DTO 自身不产生异常；构造失败（模型调用失败）由 `RunAsync` 抛 §4.2 对应 BCL 异常，不返回本 DTO                                                                                                                                                                                                                                                        |
| 日志要求     | DTO 自身不做日志；`RunAsync` 在 `agentruntime.run` span 输出 `agentruntime.model_id`（= `ModelIdUsed`）/ `agentruntime.tool_call_count`（= `ToolCalls.Count`）                                                                                                                                                                                       |
...
| 对外接口     | `[JsonPolymorphic(TypeDiscriminatorPropertyName = "$type")] [JsonDerivedType(typeof(TextPart), "text")] [JsonDerivedType(typeof(ImagePart), "image")] [JsonDerivedType(typeof(DocumentPart), "document")] public abstract record AgentMessageContentPart;` `public sealed record TextPart(string Text) : AgentMessageContentPart;` `public sealed record ImagePart(Uri ImageUri, string? MediaType = null) : AgentMessageContentPart;` `public sealed record DocumentPart(Uri DocumentUri, string FileName, string? MediaType = null) : AgentMessageContentPart;`（2026-07-08 errata：补齐 [`[JsonPolymorphic]`](https://learn.microsoft.com/dotnet/api/system.text.json.serialization.jsonpolymorphicattribute)/[`[JsonDerivedType]`](https://learn.microsoft.com/dotnet/api/system.text.json.serialization.jsonderivedtypeattribute) 特性，修复 [HD-017](../Inkwell.Core/HD-017-Inkwell.Core.Conversations.md) `ConversationMessage.ContentJson` 序列化/反序列化在 H5 会因缺少类型鉴别器而失败的缺口；`$type` 鉴别器字段名与三个子类型的字符串鉴别值均为本 HD 新增决策，未来子类型如需扩展需追加对应 `[JsonDerivedType]` 声明） |
...
| 日志要求     | `RunAsync` / `RunStreamingAsync` 完成后在 `AgentTurnResult.ModelParametersUsed` / 流式 `RunCompleted` 事件中回显最终生效值，供 [REQ-006 调试 trace](../../01-requirements/requirements.md) 查看                          |
...
### 3.7 `AgentRuntime/JsonDelegateAIFunction.cs` / `AgentToolCallRecord.cs`

> **2026-07-07 errata**：本节原为 `AgentToolDefinition`（自定义 DTO）+ `AgentToolCallRecord`。Owner 本次会话真实拍板"直接产出 `Microsoft.Extensions.AI.AIFunction`，消除重复类型"——`AgentToolDefinition` 已删除，替换为 `JsonDelegateAIFunction`（继承 [`Microsoft.Extensions.AI.AIFunction`](https://learn.microsoft.com/dotnet/api/microsoft.extensions.ai.aifunction) 抽象类，真实可被 `Microsoft.Agents.AI` 函数调用管道调用）。`AgentToolCallRecord` 字段与语义不变。以下为核实 `AITool` / `AIFunctionDeclaration` / `AIFunction` / `AIFunctionArguments`（`Microsoft.Extensions.AI.Abstractions` 10.5.1）真实成员签名后的设计。

| 字段         | 内容                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| ------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| 文件路径     | `src/core/Inkwell.Abstractions/AgentRuntime/JsonDelegateAIFunction.cs`（含 `AgentToolCallRecord`，同文件两个紧密相关的小类型，延续原 `AgentToolDefinition.cs` 的单文件多小类型共置惯例，[HD-001 §2](HD-001-Inkwell.Abstractions-foundation.md)）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| 职责         | `JsonDelegateAIFunction`：把 [REQ-007 工具调用](../../01-requirements/requirements.md) 的"工具描述 + 同进程调用委托"包装为真正可被调用的 `AIFunction`（零 Provider 依赖、零 `Microsoft.Agents.AI.*` 依赖）；`AgentToolCallRecord`：非流式路径下单次工具调用的回溯记录（[picker Q-tool-result-passthrough=A](#13-关键决策摘要)，不变）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 对外接口     | `public sealed class JsonDelegateAIFunction : AIFunction { public JsonDelegateAIFunction(string name, string description, string parametersJsonSchema, Func<string, CancellationToken, Task<string>> invokeAsync); public override string Name { get; } public override string Description { get; } public override JsonElement JsonSchema { get; } protected override ValueTask<object?> InvokeCoreAsync(AIFunctionArguments arguments, CancellationToken cancellationToken); }`；`public sealed record AgentToolCallRecord { public required string ToolCallId { get; init; } public required string ToolName { get; init; } public required string ArgumentsJson { get; init; } public required string ResultJson { get; init; } public required bool IsError { get; init; } }`（不变）                                                                                                                                                                                                                                                                             |
| 内部函数或类 | `AIFunction` 基类真实签名（已核实反射结果，非猜测）：`Name`/`Description` 为 `AITool` 上 `public virtual string` 属性（无 setter，子类通过 `override` 覆盖）；`JsonSchema` 为 `AIFunctionDeclaration` 上 `public virtual JsonElement` 属性；`InvokeCoreAsync` 为 `AIFunction` 上 `protected abstract ValueTask<object?>` 方法，是唯一必须实现的抽象成员。`JsonDelegateAIFunction` 构造期把 `parametersJsonSchema` 用 `JsonDocument.Parse(...).RootElement.Clone()` 解析缓存为 `JsonSchema`；`InvokeCoreAsync` 内部 (1) 用 `JsonSerializer.Serialize((IDictionary<string, object?>)arguments, JsonSerializerOptions)` 把框架传入的 `AIFunctionArguments`（`IDictionary<string, object?>`）序列化回 JSON 文本 `argumentsJson`；(2) `await invokeAsync(argumentsJson, cancellationToken)` 调用委托得到 `resultJson`；(3) 用 `JsonDocument.Parse(resultJson).RootElement.Clone()` 把 `resultJson` 解析为 `JsonElement` 再返回（而非直接返回 `string`）——避免宿主框架把 `resultJson` 当作**字符串值**二次 JSON 序列化（产生多一层转义引号），这是本次桥接实现的关键正确性点 |
| 输入数据     | 构造参数：`name` / `description` / `parametersJsonSchema`（[JSON Schema](https://json-schema.org/) 文本）/ `invokeAsync` 委托（由 `Inkwell.Core.Tools` 业务层提供实现，桥接到具体工具执行逻辑，签名不变，仍是 `Func<string, CancellationToken, Task<string>>`）；`InvokeCoreAsync` 运行期入参 `AIFunctionArguments arguments`（由 `Microsoft.Agents.AI` 函数调用管道构造并传入，本类型不感知该管道）；`AgentToolCallRecord`：由实现层在工具调用完成后填充（不变）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
| 输出数据     | `JsonDelegateAIFunction` 实例；`InvokeCoreAsync` 返回 `ValueTask<object?>`（装箱的 `JsonElement`）；`AgentToolCallRecord` 实例（不变）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                 |
| 依赖模块     | `Microsoft.Extensions.AI.{AIFunction,AIFunctionArguments}`（外部包，已在依赖白名单）/ `System.Text.Json`                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                               |
| 错误处理     | 构造期：`name` / `description` / `parametersJsonSchema` 为 null/empty → `ArgumentException`；`invokeAsync` 为 `null` → `ArgumentNullException`；`parametersJsonSchema` 非合法 JSON → `JsonException`（`JsonDocument.Parse` 原生抛出，本类不捕获包装）。运行期：`InvokeCoreAsync` 是对委托的薄转发，委托内部抛出的异常**不**在本类捕获转换为"错误结果"——与原 `AgentToolDefinition` 设计不同，`AgentToolCallRecord.IsError` 语义现由调用方（`Microsoft.Agents.AI` 函数调用管道或 `Inkwell.Core.Tools` 委托实现自身）决定异常是否被捕获转换，本 HD 不代为约束该行为差异，已列入 §11 待补                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
| 日志要求     | `agentruntime.tool_call_count` 字段统计 `ToolCalls.Count`（不变）；`ArgumentsJson` / `ResultJson` 原文**不得**进 OTel（可能含业务敏感参数，详 §7.2），仅记录 `ToolName` + 是否出错                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                     |
| 测试要求     | `JsonDelegateAIFunctionTests.cs`：(1) 构造参数校验（必填非空 / 委托非空 / 非法 JSON Schema 抛 `JsonException`）；(2) `Name` / `Description` / `JsonSchema` 正确暴露；(3) `InvokeCoreAsync` 正确把委托 `resultJson` 转换为 `JsonElement`（非二次转义字符串，需断言反序列化结构而非仅字符串相等）；(4) 委托抛异常时异常原样从 `InvokeCoreAsync` 传播（不被吞没）。`AgentToolCallRecordTests.cs`：(1) 必填字段校验；(2) `IsError = true` 场景的构造合法性；(3) record equality（不变）                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
...
| 职责         | `RunStreamingAsync` 产出的流式事件封闭子类型族；1:1 对应 [ADR-012](../../03-architecture/adr/ADR-012-client-server-protocol-rest-agui.md) AG-UI 的 message / tool_call / state_delta / lifecycle 四大类事件（[picker Q-streaming-event-shape=A](#13-关键决策摘要)），供 `Inkwell.WebApi` 的 AG-UI hosting 层 1:1 映射转发                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
- **预期返回值（不是异常，调用方按值判断）**：
  ...
  - `RunStreamingAsync` 遇到不可恢复错误 → 产出 `RunError` 事件后正常结束枚举（**不**抛异常，详 §3.8）
...
- **参数 / 取消错误**（调用方 bug，应在测试期暴露）：
  ...
  - `OperationCanceledException`：全部方法响应 `ct`（[HD-001 §4.3](HD-001-Inkwell.Abstractions-foundation.md#43-取消传播)）；`RunStreamingAsync` 的流式枚举在 `ct` 触发时立即终止枚举（同 [HD-005 `DequeueAsync`](HD-005-Inkwell.Abstractions-queue-port.md#42-bcl-异常分类业务失败-vs-程序错误) 取消惯例）；用户主动 `CancelRunAsync` 触发的中断，实现层内部通过 `CancellationTokenSource.Cancel()` 使正在执行的 `RunAsync` / `RunStreamingAsync` 抛 `OperationCanceledException`，与被动 `ct` 取消走同一异常路径（详 §4.4 CancelRunAsync 内部机制）

### 4.3 OTel span / 字段

每个方法在实现层（`Inkwell.Core.AgentRuntime`）按 [picker Q-otel](#13-关键决策摘要) 输出 span：

- `agentruntime.run` ← `RunAsync`
- `agentruntime.run_streaming` ← `RunStreamingAsync`（每次 `AgentRunEvent` 产出各起一个子 span，而非整个枚举生命周期一个 span，与 [HD-005 `queue.dequeue`](HD-005-Inkwell.Abstractions-queue-port.md#43-otel-span--字段) 处理方式一致）
...
- `agentruntime.operation_outcome`：值域按方法区分——
  - `RunAsync` / `RunStreamingAsync`：`completed` / `failed` / `cancelled`
...
- `RunAsync` / `RunStreamingAsync` / `CancelRunAsync` ↔ §5.1 异步方法以 `Async` 结尾
...
- `RunStreamingAsync` 是流式签名 ↔ [HD-001 §5.2 流式签名约定](HD-001-Inkwell.Abstractions-foundation.md#52-签名)（`IAsyncEnumerable<T>` + `[EnumeratorCancellation]`）
...
| 方法                | 预算类型        | P50    | P99     | 备注                                                                  |
| ------------------- | --------------- | ------ | ------- | --------------------------------------------------------------------- |
| `RunAsync`          | facade overhead | < 50ms | < 200ms | 不含模型推理延迟；仅统计请求组装 + 工具调用编排的框架开销             |
| `RunStreamingAsync` | facade overhead | < 50ms | < 200ms | 同上，指首个事件产出前的框架开销，不含模型首 token 延迟               |
...
- `JsonDelegateAIFunction` 构造时传入的 `invokeAsync` 委托由 `Inkwell.Core.Tools` 业务层提供，工具执行期的权限校验（如某工具是否需要额外授权）是该业务层职责，本 HD 不重复约束
...
- 6 私有 + 5 OTel 标准 `exception.*` 字段进 OTel；本 HD 不锁告警规则（H4 [TestCaseAuthor](../../../.github/agents/h4-test-case-author.agent.md) 反推时锁），但建议告警维度：
  ...
  - **跨事件 trace correlation**：`agentruntime.run_id` 应与 [REQ-014 trace 全链路](../../01-requirements/requirements.md) 的顶层 trace 关联，使同一 Run 的 `agentruntime.run_streaming` 子 span、工具调用、模型调用（MAF 自带 OTel instrumentation，[ADR-003 后果](../../03-architecture/adr/ADR-003-agent-engine-microsoft-agent-framework.md)）在同一条 trace 内呈现
...
- 覆盖率门槛：DTO 类（`AgentRunRequest` / `AgentChatMessage` / `AgentMessageContentPart` / `AgentModelParameters` / `JsonDelegateAIFunction` / `AgentToolCallRecord` / `AgentRunEvent`）≥ 95%；`AgentRuntimeOptions` + Validator ≥ 90%；`IAgentRuntimeContractTests` 仅锁 ABI ≥ 100%
...
- **多 Agent 编排场景下 `IAgentRuntime` 的复用方式**：[REQ-012](../../01-requirements/requirements.md) DAG 编排如何调用多个 `IAgentRuntime.RunAsync`（每节点一次调用 vs 批量调度）——留 `Inkwell.Core.Orchestrations` 业务 HD 决定，本 HD 仅保证单次 Run 语义完整
...
- **（2026-07-07 errata 新增）`JsonDelegateAIFunction.InvokeCoreAsync` 抛出异常的处理归属未定**：与已删除的 `AgentToolDefinition` 设计不同，本类不再自动把委托异常转换为 `AgentToolCallRecord.IsError = true`——异常处理归属（`Microsoft.Agents.AI` 函数调用管道 vs `Inkwell.Core.Tools` 委托实现自身捕获）留待 `Inkwell.Core.AgentRuntime` / `Inkwell.Core.Tools`（[HD-016](../Inkwell.Core/HD-016-Inkwell.Core.Tools.md)）后续 HD 明确，本 HD 不预判
- **（2026-07-07 errata 新增）与 [HD-016](../Inkwell.Core/HD-016-Inkwell.Core.Tools.md) 的跨 HD 类型不一致**：本次任务范围不含 HD-016，`IToolBindingResolver.ResolveAsync` 返回类型仍为 `IReadOnlyList<AgentToolDefinition>`（已删除类型），与本 HD `AgentRunRequest.Tools: IReadOnlyList<AIFunction>?` 不再匹配，需后续对 HD-016 单独发起 errata 修复，本 HD 不代为修改
...
| 字段                        | 选定值                                                                                                                                                                                                                | picker 时间 |
| --------------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------- |
| Q-facade-scope              | A：3 方法（RunAsync / RunStreamingAsync / CancelRunAsync），不引入端口层 Session 生命周期方法                                                                                                                         | 2026-07-05  |
...
| Q-tool-definition-shape     | A：`AgentToolDefinition` 用 `string` 载 `ArgumentsJson` / `ResultJson`，与 `AgentRunEvent` 同形态（**已被 2026-07-07 errata 取代**：`AgentToolDefinition` 删除，改用 `JsonDelegateAIFunction : AIFunction`，详 §3.7） | 2026-07-05  |
