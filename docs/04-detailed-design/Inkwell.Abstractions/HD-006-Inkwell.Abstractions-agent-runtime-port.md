...
- **不在内**：
  ...
  - `AgentSession` / 对话历史的持久化与重建策略（[`Inkwell.Conversations`](../Inkwell.Core/HD-017-Inkwell.Core.Conversations.md) 业务 HD 负责提供历史消息查询接口，由未起草的 `Inkwell.WebApi` 在调用前查询历史并组装成 `AgentRunRequest.Messages`，2026-07-08 errata 精确化措辞；本 HD 端口不反向依赖 `IPersistenceProvider`，[picker Q-run-request-shape=A](#131-起草期-picker-决策2026-07-05)）
...
| 职责         | 一次 Run 调用的全部已解析上下文；未起草的 `Inkwell.WebApi` 负责在调用前查好 Agent 配置（`Inkwell.Core.Agents`）/ 对话历史（`Inkwell.Core.Conversations`）/ 工具注册表（`Inkwell.Core.Tools`）并组装本 DTO（2026-07-08 errata 精确化消费方措辞——业务命名空间自身只提供查询能力，不直接调用 `IAgentRuntime`）（[picker Q-run-request-shape=A](#13-关键决策摘要)），端口本身不反向依赖 `IPersistenceProvider`                                                                                                                                                                                                                                                                                                                |
