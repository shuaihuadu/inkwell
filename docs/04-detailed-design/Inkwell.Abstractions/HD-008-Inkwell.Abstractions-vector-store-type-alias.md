...
<!-- 中文 + 英文混排长表格在 markdownlint 列宽计算下字面对齐 ≠ 视觉对齐（详 /memories/markdown-lint.md，与 HD-004 ~ HD-007 同处理方式），表格仍按 docs-style §3 视觉对齐维护，机械 MD060 不予执行。 -->
...
| ID                     | 决策                                                                                                                                                                                                                                | 出处                                                                                                                                                                                                                                                                                                                                                                    |
| ---------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Q1-embedding-topology  | `UseAzureOpenAIEmbeddings(...)` 实现 + `AzureOpenAIEmbeddingOptions` 落在 `Inkwell.Core/VectorStore/`——单实现拓扑，不设独立 `providers/*` csproj                                                                                    | picker 2026-07-06；与 [HD-006 Q-implementation-topology](HD-006-Inkwell.Abstractions-agent-runtime-port.md) / [HD-007 Q-implementation-topology](HD-007-Inkwell.Abstractions-audit-logger-port.md#13-关键决策摘要) 同款                                                                                                                                                 |
...
**文件计数**：HD-008 新增 2 个 `*.cs`（`VectorStore/` 2：`VectorStoreOptions.cs` + `VectorStoreOptionsValidator.cs`）；`GlobalUsings.cs` / `Options/InkwellOptions.cs` 为既有文件追加行，不计入新增文件数。Abstractions csproj 累计 11（HD-001）+ 8（HD-002 本体）+ 7（HD-003）+ 4（HD-004）+ 4（HD-005）+ 10（HD-006）+ 7（HD-007）+ 2（HD-008）= **53** 个 `*.cs` + 1 个 `.csproj`。
...
| 职责         | 在根 `InkwellOptions` 追加 `VectorStore` 子 Options 槽位，与 `Persistence` / `FileStorage` / `Cache` / `Queue` / `AgentRuntime` / `Audit` 六个既有槽位同款（HD-002 ~ HD-007 依次追加的同一模式，本 HD 是第七次追加） |
| 对外接口     | 追加字段：`public VectorStoreOptions VectorStore { get; init; } = new();`（插入在 `AuditLoggerOptions Audit` 之后）                                                                                                  |
...
**本 HD 不新增任何包裹该接口的 Inkwell 门面类型**（如 `IEmbeddingProducer`）——ADR-020 字面表述是业务层直接消费 `IEmbeddingGenerator<,>`，本 HD 遵此表述。该表述与 [AGENTS.md §3.2](../../../AGENTS.md) "业务命名空间只能依赖 `Inkwell.Abstractions` + BCL"的依赖纯度原则之间存在张力（`Microsoft.Extensions.AI.Abstractions` 既非 BCL 也非 `Inkwell.Abstractions`）；Owner 已在 chat picker（2026-07-06）确认：`IEmbeddingGenerator<string, Embedding<float>>` **允许** `Inkwell.Core.KnowledgeBase` / `.Memory` 业务命名空间直接注入，比照本 HD 自身对 `Microsoft.Extensions.VectorData.Abstractions` 的处理先例（[§1.1](#11-职责) / [ADR-020](../../03-architecture/adr/ADR-020-vector-store-microsoft-extensions-vectordata.md)），不需要 Inkwell 自己包一层门面接口（[§13 Q5](#13-决策记录)）。该决策依据本 HD 自身既有先例作出，**不**改动 [AGENTS.md §3.2](../../../AGENTS.md) 依赖白名单原文——`AGENTS.md` 现状并未字面列出 `Microsoft.Extensions.VectorData.*` 的业务命名空间例外条款，故本 HD 同样不生造 `Microsoft.Extensions.AI.*` 的例外条款字面；若未来需要在 `AGENTS.md` 层面正式登记该例外，需另行发起 ADR / `AGENTS.md` 修订，不在本 HD 权限范围内。

## 7. 测试策略

- `VectorStoreOptionsTests.cs` / `VectorStoreOptionsValidatorTests.cs`（[§3.1](#31-vectorstorevectorstoreoptionscs) / [§3.2](#32-vectorstorevectorstoreoptionsvalidatorcs)）：本 HD 范围内的唯一运行期可测对象是 Options 绑定与校验；无运行期方法（[§1.2](#12-范围)），故无契约测试 / 无性能基准（与 HD-002 ~ HD-007 均定义了 facade 方法不同）
...
- [ADR-017 Ports & Adapters](../../03-architecture/adr/ADR-017-backend-module-topology-ports-and-adapters.md)：端口层零外部包依赖规则（本 HD 遵此规则，仅使用已白名单的 `Microsoft.Extensions.VectorData.Abstractions`）
...
| Q5-embedding-generator-injection | A：`IEmbeddingGenerator<string, Embedding<float>>` 允许 `Inkwell.Core.KnowledgeBase` / `.Memory` 业务命名空间直接注入，比照本 HD 自身对 `Microsoft.Extensions.VectorData.Abstractions` 的处理先例，不新增 Inkwell 门面接口；B：新增 `IEmbeddingProducer` 门面接口维持业务命名空间"只依赖 `Inkwell.Abstractions`"的纯度 | A | Owner chat picker（2026-07-06）：与 `Microsoft.Extensions.VectorData.Abstractions` 同等对待，避免为语义等价的官方抽象重复造门面；B 增加维护成本且偏离 [ADR-020](../../03-architecture/adr/ADR-020-vector-store-microsoft-extensions-vectordata.md) 现有字面表述 |
