...
```
Microsoft.Extensions.VectorData.Abstractions   <-- 统一抽象 NuGet 包
    |
    v
...
VectorStoreCollectionDefinition         集合 Schema 定义
    |
    v  实现（SK Connectors 生态）
InMemory / Qdrant / AzureAISearch / Postgres(pgvector) ...
```
...
IEmbeddingGenerator -- Embedding 生成（来自 Microsoft.Extensions.AI）：
...
MAF 的 RAG 知识检索组件，不依赖 VectorStore，接受通用搜索委托：
...
public static class VectorStoreServiceCollectionExtensions
{
    public static InkwellCoreBuilder UseInMemoryVectorStore(this InkwellCoreBuilder builder)
    {
        ...
        {
            var embedding = sp.GetRequiredService<IEmbeddingGenerator<string, Embedding<float>>>();
        ...
        this InkwellCoreBuilder builder, string host, int port = 6334)
    {
        ...
        {
            var embedding = sp.GetRequiredService<IEmbeddingGenerator<string, Embedding<float>>>();
            return new QdrantVectorStore(new QdrantClient(host, port),
        ...
        this InkwellCoreBuilder builder, IConfiguration configuration)
    {
        ...
        builder.Services.AddSingleton<IEmbeddingGenerator<string, Embedding<float>>>(sp =>
        {
            AzureOpenAIClient client = string.IsNullOrWhiteSpace(options.Embedding.ApiKey)
                ...
                : new(new Uri(options.Embedding.Endpoint),
                       new AzureKeyCredential(options.Embedding.ApiKey));
            return client.GetEmbeddingClient(options.Embedding.DeploymentName)
                .AsIEmbeddingGenerator();
        });
        return builder;
    }
...
只封装注册便利方法（UseInMemoryVectorStore / UseQdrantVectorStore 等），不封装抽象接口。与 Inkwell 现有的 UseAzureOpenAI、UseInMemoryPersistence、UseSqlServerPersistence 风格一致。
