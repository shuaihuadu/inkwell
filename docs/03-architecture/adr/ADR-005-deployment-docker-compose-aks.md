...
<!-- 2026-05-10 [ADR-019](./ADR-019-process-topology-webapi-worker-split.md) 引入后后端拆为 `Inkwell.WebApi` + `Inkwell.Worker` 双进程：Compose 加 `worker` service；Helm 加 `Deployment: inkwell-worker` + HPA(`queue_depth`)。本文件 §决策 / §后果 已同步。 -->
