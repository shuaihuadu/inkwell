...
> **2026-07-13 errata（Owner 明确修订）**：JSON 持久化不再取文本类型最小公倍数。CLR 契约继续使用 `string` + `System.Text.Json` 序列化，但物理列必须使用 Provider 原生 JSON 类型：PostgreSQL 17 使用 `jsonb`，SQL Server 2025 使用 `json`。两套 Provider 通过独立 migration 管理类型差异；已有文本列迁移时必须校验并转换现存 JSON，非法数据使 migration 明确失败。此 errata 仅放开 JSON 列类型，不放开其他未评审的 Provider-specific schema 优化。
