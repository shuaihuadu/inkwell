...
# ADR-021 EFCore Persistence 共享层 + 三 Provider 多层 csproj 布局

## 上下文

[ADR-004 §决策](./ADR-004-data-store-provider-switchable-ef-core.md) 锁定「`IPersistenceProvider` 抽象 + 三 Provider 实现（InMemory / SQL Server 2025 / PostgreSQL 17）+ Code First + EF Migration」，[ADR-017 §3.1 csproj 树](./ADR-017-backend-module-topology-ports-and-adapters.md) 把三 Provider 直接列在 `src/core/providers/` 下与 Cache / FileStorage / Queue / VectorStore Provider 平级——但**未锁定**：

1. **Entity 类的归属**：业务 Entity（`Agent` / `Conversation` / `Run` / `Trace` / `AuditLog` / `KnowledgeBase` / `MemoryItem` / `Skill` / `Tool` / `Trigger` / `Orchestration` / `Version` / `User` 等共 ~30 个 Entity）属于关系数据建模，**不应在三 Provider csproj 中重复定义**——重复会导致 schema 漂移、Migration 不一致、新加字段时三处改三遍的 DRY 灾难。

2. **`IPersistenceProvider` 实现位置**：`EfCorePersistenceProvider`（注入 `InkwellDbContext` 的具体实现）应该出现一份还是三份？三份会出现「跨 Provider 切换 = 切实现」错误模型——实际上业务代码逻辑是同一份，仅 `DbContextOptionsBuilder` 选项不同。
...
5. **InMemory Provider 的事实约束**：[`Microsoft.EntityFrameworkCore.InMemory`](https://learn.microsoft.com/ef/core/providers/in-memory/) 不支持 Migration——它通过 `EnsureCreated()` 重建内存 schema，不参与 Migration tooling。Owner 原始描述「InMemory 的 Migration」事实上不可行，本 ADR 把它精化为「InMemory csproj 仅含 DbContextOptionsBuilder 配置 + EnsureCreated/EnsureDeleted helper + 测试种子辅助」。

Owner 在本 ADR 起草会话中通过 picker 拍板：D1 = A（共享层独立 csproj 与三 Provider 平级）；D2 = B（DataSeed 通过 Builder DSL `.AutoSeedOnStartup(bool)` 开关，default true）；D3 = A（SqlServer / Postgres 各自 `Migrations/`，InMemory 仅 EnsureCreated）。

## 决策

**EFCore Persistence 拓扑变为 4 层 csproj**——共享 base + 三 final adapter Provider，物理位于 `src/core/providers/` 下平级：
...
├── Inkwell.Persistence.EFCore.InMemory/            ← final adapter（仅 DbContextOptionsBuilder.UseInMemoryDatabase）
│   ├── DbContexts/InMemoryInkwellDbContext.cs      ← 继承 base 添加 InMemory-specific 配置
│   ├── DependencyInjection/UseInMemoryDatabase.cs  ← Builder DSL `.UseInMemoryDatabase()`
│   └── EnsureCreatedHelper.cs                      ← startup-time schema 创建（替代 Migration）
...
后端总 csproj 数 [ADR-020](./ADR-020-vector-store-microsoft-extensions-vectordata.md) 锁定的 12 → **13**（providers/ 8 → 9）。
...
- **`Inkwell.Persistence.EFCore.{InMemory,SqlServer,Postgres}` 允许引用 `Inkwell.Persistence.EFCore`（同 providers/ 下的 base csproj）**——这是 P&A 中"shared adapter base + final adapter"的标准分层，不是越权依赖。
...
- 三 final adapter csproj **不实现** `IPersistenceProvider`，仅注册 `DbContextOptions<InkwellDbContext>`：
  - InMemory: `services.AddDbContext<InkwellDbContext>(o => o.UseInMemoryDatabase("inkwell"))`
...
H3 [HD-001 Inkwell.Abstractions] 必须锁定：(1) `PersistenceOptions` 字段（含 `AutoSeedOnStartup` / `MigrationTimeoutSeconds` / `CommandTimeoutSeconds`）；(2) `UseInMemoryDatabase` / `UseSqlServer` / `UsePostgres` 三个扩展方法签名（位于各 final adapter csproj，但 fluent return type `IInkwellBuilder` 在 Abstractions 中定义）；(3) `AutoSeedOnStartup(bool)` 扩展方法签名。

### Migration / DataSeed 启动行为

- **Migration**：[ADR-019](./ADR-019-process-topology-webapi-worker-split.md) 锁定的 `Inkwell.WebApi` 启动时（**仅 WebApi**，Worker 跳过）由 `MigrationRunner` 包装调 `dbContext.Database.MigrateAsync()`——SqlServer / Postgres 走 Provider-specific Migration；InMemory 走 `dbContext.Database.EnsureCreatedAsync()`（实现差异由 final adapter csproj 注册的 `IDbContextInitializer` 服务区分）。
...
>
> 上方「DataSeed」一条不受影响——`InkwellSeeder.SeedAsync()` 仍在 `Inkwell.WebApi` 启动时运行（`.AutoSeedOnStartup` 开关不变），但前提从「随 `MigrationRunner` 完成 Migration 后触发」改为「确认 CI/CD 已将 schema 迁移到位」；InMemory Provider `EnsureCreatedAsync()`（dev / unit test）不受影响，不涉及生产发布流程。
>
> 触发原因：H3 [HD-011 SqlServer final adapter](../../04-detailed-design/Inkwell.Persistence.EFCore/HD-011-Inkwell.Persistence.EFCore.SqlServer-adapter.md) 起草期发现的生产安全考量；2026-07-06 经 Owner chat picker 拍板确认。同步 errata：[ADR-019](./ADR-019-process-topology-webapi-worker-split.md)、[risk-analysis.md RISK-017](../risk-analysis.md)。
...
- **放弃理由**：(1) 与 Owner 直觉描述不一致——Owner 期望 4 个 csproj 平级；(2) Inkwell.Persistence.EFCore 仍是**适配器实现**而非业务核心，与 `Inkwell.Core`（业务逻辑层）平级会暗示它是核心层；(3) 引入 P&A "二层 vs 三层" 的方法论辩论，v1 不值；(4) 与未来若 FileStorage / Cache / Queue / VectorStore 也出现 base 时，那一刻新 csproj 该放哪儿就更难统一

### 备选 B（D1 = C）：不抽公共 csproj，把 Entity 放 `Inkwell.Abstractions`

- **放弃理由**：(1) 违反 [ADR-017 §模块依赖规则](./ADR-017-backend-module-topology-ports-and-adapters.md)「`Inkwell.Abstractions` 零外部包依赖」——Entity 类需 `[Key]` / `[Required]` / 导航属性 → 拉进 `System.ComponentModel.DataAnnotations` 与 EF Core attribute（如 `[Owned]`）；(2) Abstractions 应该是端口（接口）层，不该含具体 Entity 数据建模——concept smell；(3) 三 Provider 各自仍需要 `OnModelCreating` 共享配置 + DataSeed 共享逻辑，问题没解决只是把重复代码从 Entity 转移到 DbContext 配置
...
- csproj 数 12 → 13；编译图 += 1 节点
- InMemory csproj 失去"Migration 测试"能力（事实约束）——靠 SqlServer / Postgres 双 Provider matrix 兜底覆盖 Migration 正确性
- 三 final adapter csproj 几乎对称（仅 NuGet 包不同）——未来加新 Provider（如 [MySQL Pomelo](https://github.com/PomeloFoundation/Pomelo.EntityFrameworkCore.MySql)）路径明确
...
```bash
# 检查共享 base csproj 是否被三 final adapter 引用
grep -rn 'ProjectReference.*Inkwell\.Persistence\.EFCore\b' src/core/providers/

# 检查三 final adapter 是否各自有 Migrations/（InMemory 应无）
...
# 检查 InMemory csproj 是否引入了不该有的 Migration NuGet
grep -rn 'Microsoft\.EntityFrameworkCore\.Design' src/core/providers/Inkwell.Persistence.EFCore.InMemory/
```

## 状态

`accepted` — 2026-05-10。Owner 在本 ADR 起草会话中通过 3 个 picker 拍板：D1 = A（共享层 csproj 与三 Provider 平级在 `providers/`）；D2 = B（DataSeed 通过 Builder DSL 开关 default true）；D3 = A（Migration 在 SqlServer / Postgres 各自 csproj，InMemory 仅 EnsureCreated）。本 ADR 是 [ADR-004 §决策](./ADR-004-data-store-provider-switchable-ef-core.md) + [ADR-017 §3.1 csproj 树](./ADR-017-backend-module-topology-ports-and-adapters.md) 的精化（refinement），**不 supersede** 任一前置 ADR。
