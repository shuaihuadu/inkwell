...
>
> **2026-07-14 增量更新**：PostgreSQL 支持基线由 17 升级到 **18**，当前固定补丁为 `18.4`；不改变 EFCore family 拓扑、`jsonb` 映射或两套 Provider-specific Migration 的既有边界。生产环境从 17 升级时必须执行 PostgreSQL major upgrade 流程。

## 上下文

[ADR-004 §决策](./ADR-004-data-store-provider-switchable-ef-core.md) 锁定「`IPersistenceProvider` 抽象 + 两 Provider 实现（SQL Server 2025 / PostgreSQL 18）+ Code First + EF Migration」，[ADR-017 §3.1 csproj 树](./ADR-017-backend-module-topology-ports-and-adapters.md) 把两 Provider 直接列在 `src/core/providers/` 下与 Cache / FileStorage / Queue / VectorStore Provider 平级——但**未锁定**：
...
> **2026-07-13 errata（Owner 明确修订）**：JSON 持久化不再取文本类型最小公倍数。CLR 契约继续使用 `string` + `System.Text.Json` 序列化，但物理列必须使用 Provider 原生 JSON 类型：PostgreSQL 18 使用 `jsonb`，SQL Server 2025 使用 `json`。两套 Provider 通过独立 migration 管理类型差异；已有文本列迁移时必须校验并转换现存 JSON，非法数据使 migration 明确失败。此 errata 仅放开 JSON 列类型，不放开其他未评审的 Provider-specific schema 优化。

- 除 JSON 列和既有索引策略外，v1 两 Provider 的 schema 仍取最小公倍数，UTC datetime 统一使用 `DateTimeOffset`。
- JSON 的 CLR 属性保持 `string`，避免 Provider 类型泄漏到共享 Entity 和业务 Model；`InkwellDbContext` 根据当前 EF Provider 将这类属性映射到 PostgreSQL `jsonb` 或 SQL Server `json`。
- 索引策略可按 Provider 优化：SQL Server 用 `INCLUDE`，PostgreSQL 用 `WHERE` 过滤索引；其他 Provider-specific 类型或优化仍需独立决策。
...
- 除原生 JSON 列与既有索引策略外，v1 Provider-specific schema 优化仍被锁定为「最小公倍数」，性能调优受限
...
ls src/core/providers/Persistence/Inkwell.Persistence.EFCore.SqlServer/Migrations/
ls src/core/providers/Persistence/Inkwell.Persistence.EFCore.Postgres/Migrations/

# 检查仓库中是否残留 InMemory adapter csproj（应不存在）
test ! -d src/core/providers/Persistence/Inkwell.Persistence.EFCore.InMemory && echo OK || echo "BAD: InMemory adapter 仍存在"
```
