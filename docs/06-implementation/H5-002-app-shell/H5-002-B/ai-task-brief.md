...
将认证后的 Header、分组导航、页面容器和登出从 Agent 功能组件抽取为公共 AppShell，并实现工作区、资源中心和仅 Super 用户可见的系统管理导航。

## 2. 不做范围

- 不改 Agent 列表、创建、聊天或数据查询行为；归 H5-003/H5-005。
- 不实现统一连接状态、全局错误映射和主题设置；归 H5-002-C。
...
- `src/app/desktop/electron/**`、preload、IPC 和网络契约。
