...
- Agent Conversation 三模型、Repository、Service、REST、EF Core Mapping 与双 Provider Initial Migration。
- MAF `MapAGUI` 协议入口和 `InkwellChatHistoryProvider` 基础实现。
...
- H5-005-A：Conversation 三模型、Repository、Service、REST 与双 Provider Migration 已实现，补齐验证和实施记录。
- H5-005-B：基于现有 `MapAGUI` 与 `InkwellChatHistoryProvider`，用真实 TypeScript SDK 发包验证 MAF AG-UI 和完整消息快照契约；实际 Agent Session 恢复方案继续暂缓，不在本任务中臆造。
