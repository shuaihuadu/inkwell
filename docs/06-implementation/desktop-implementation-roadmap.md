...
- Agent 草稿保存、发布、跨刷新未发布修改状态、版本列表、完整 Snapshot diff 与回滚。

已知偏差：

- 暂无与 REQ-015 草稿/发布和版本管理链路相关的已知偏差。
...
flowchart TD
    ...
    CHATDATA[H5-005-A 会话数据与 REST 已实现]
...
H5-002-A/B 已在 `726ebd6` 实现依赖升级、公共 AppShell、权限导航、占位入口、主题设置、关于弹层和原型对齐锁屏；H5-002-C 已实现真实网络状态、写请求门禁与全局错误映射，等待 Owner 评审。
...
覆盖 UI-004 和 REQ-003～009、REQ-010 的记忆配置部分、REQ-015 的草稿/发布动作。其中 H5-004-D 草稿保存、发布和未发布修改状态已实现：

- H5-004-A：基础属性、Instructions、模型与参数。
- H5-004-B：工具与 Skills 绑定。
- H5-004-C：知识库与长期记忆配置。
- H5-004-D：草稿保存、发布和只读权限状态已实现。
...
覆盖 UI-008 和 REQ-015 的版本列表、完整 Snapshot diff 与回滚，现已实现。发布与回滚由 Core Service 编排，PostgreSQL 使用 JSONB 保存不可变 Snapshot；桌面端回滚后恢复实际配置字段并刷新版本状态。
