...
public interface IAgentVersionService
{
    ...
    /// </summary>
    /// <param name="agentId">Agent 标识。</param>
    /// <param name="snapshot">完整草稿快照。</param>
    /// <param name="actorUserId">操作用户标识。</param>
    /// <param name="changeSummary">变更摘要。</param>
    /// <param name="cancellationToken">取消令牌。</param>
    /// <returns>保存后的草稿版本。</returns>
    Task<AgentVersion> SaveDraftAsync(Guid agentId, AgentSnapshot snapshot, Guid actorUserId, string? changeSummary = null, CancellationToken cancellationToken = default);
    ...
    Task<AgentVersion> PublishDraftAsync(Guid agentId, Guid actorUserId, CancellationToken cancellationToken = default);
