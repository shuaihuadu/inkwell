...
const parseSkillMarkdown = (content: string): Pick<SkillUploadPreview, "name" | "description"> => {
    ...
    const readField = (field: string): string =>
        frontmatter[1]
            ...
            .find((match) => match?.[1].trim().toLocaleLowerCase() === field)?.[2]
            .trim() ?? "";
...
const createUploadPreview = async (file: File): Promise<SkillUploadPreview> => {
    if (file.name.toLocaleLowerCase().endsWith(".md")) {
        return { ...parseSkillMarkdown(await file.text()), references: 0, assets: 0, scripts: 0 };
    }
    const archive = await JSZip.loadAsync(file);
    const files = Object.values(archive.files).filter((entry) => !entry.dir);
    const markdownFiles = files.filter((entry) => /(^|\/)SKILL\.md$/i.test(entry.name));
    if (markdownFiles.length !== 1) throw new Error("压缩包必须包含且只包含一个 SKILL.md");
    const markdown = markdownFiles[0];
    const root = markdown.name.slice(0, -"SKILL.md".length);
    const relativePaths = files
        .filter((entry) => entry !== markdown)
        .map((entry) => {
            if (!entry.name.startsWith(root)) throw new Error("所有文件必须位于 SKILL.md 所在文件夹内");
            return entry.name.slice(root.length);
        });
    const count = (folder: string): number =>
        relativePaths.filter((path) => path.toLocaleLowerCase().startsWith(`${folder}/`)).length;
    if (relativePaths.some((path) => !/^(references|assets|scripts)\//i.test(path))) {
        throw new Error("包内文件只能放在 references、assets 或 scripts 文件夹");
    }
    return {
...
export function SkillManagement() {
    ...
    const [uploadPreview, setUploadPreview] = useState<SkillUploadPreview | null>(null);
    ...
    const skills = (skillsQuery.data ?? []).filter((skill) => {
        const matchesText = `${skill.name} ${skill.description} ${ownerLabel(skill)}`
            .toLocaleLowerCase()
    ...
    const saveSkill = async (): Promise<void> => {
        ...
        const request: AgentSkillUpdateRequest = {
            ...values,
            rowVersion: selectedSkill.rowVersion,
        };
        setSaving(true);
        try {
            const updated = await desktopApi.updateSkill(selectedSkill.id, request);
            queryClient.setQueryData<AgentSkillDefinition[]>(["skills"], (current) =>
    ...
    const updateUploadFiles = async (files: UploadFile[]): Promise<void> => {
        ...
        try {
            ...
            messageApi.error(error instanceof Error ? error.message : "Skill 文件无法解析");
        } finally {
            setPreviewing(false);
        }
    };
    ...
    return (
        <DataListPage<AgentSkillDefinition>
            ...
            errorMessage={skillsQuery.isError ? "Skills 加载失败，请稍后重试" : undefined}
        ...
        >
            ...
            >
                {selectedSkill && (
                    <Form form={form} layout="vertical" disabled={!editing}>
                        ...
                        <Form.Item label="SKILL.md 内容" name="content" rules={[{ required: true, message: "请输入内容" }]}>
                            <Input.TextArea rows={14} className="inkwell-skill-editor" />
                        </Form.Item>
                        ...
                        <Space size={24} wrap>
                            <Typography.Text type="secondary">所有者：{ownerLabel(selectedSkill)}</Typography.Text>
                            <Typography.Text type="secondary">引用：{selectedSkill.referenceFileUris.length} 个（只读）</Typography.Text>
                            <Typography.Text type="secondary">素材：{selectedSkill.assetFileUris.length} 个（只读）</Typography.Text>
                            <Typography.Text type="secondary">脚本：{selectedSkill.scriptFileUris.length} 个（只读）</Typography.Text>
                        </Space>
                    </Form>
                )}
            </Drawer>
            <Modal
                ...
                okButtonProps={{ disabled: uploadPreview === null || previewing }}
                onCancel={() => {
                    setUploadOpen(false);
                    setUploadFiles([]);
                    setUploadPreview(null);
            ...
            >
                <Upload.Dragger
                    ...
                    onChange={({ fileList }) => void updateUploadFiles(fileList)}
                >
                    <p className="ant-upload-drag-icon"><InboxOutlined /></p>
                    <p className="ant-upload-text">选择 Skill 文件夹压缩包或 SKILL.md</p>
                    <p className="ant-upload-hint">
                        名称和描述读取自 SKILL.md 的 YAML frontmatter，上传后可在详情中编辑。支持 references/、assets/ 和 scripts/。
                    </p>
                </Upload.Dragger>
                {previewing && (
                    <Typography.Text type="secondary">正在解析 SKILL.md...</Typography.Text>
                )}
                {uploadPreview && (
                    <div style={{ marginTop: 16 }}>
                        <Typography.Title level={5}>SKILL.md 解析预览</Typography.Title>
                        <Descriptions size="small" column={1} bordered>
                            ...
                            <Descriptions.Item label="包内资源">
                                {uploadPreview.references} 个 references · {uploadPreview.assets} 个 asset · {uploadPreview.scripts} 个 scripts
                            </Descriptions.Item>
                        </Descriptions>
                    </div>
                )}
