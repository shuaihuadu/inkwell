...
public sealed class RedisQueueProviderTests
{
    ...
    [TestMethod]
    public async Task EnqueueAsync_Then_DequeueAsync_Delivers_Message()
    {
    ...
    [TestMethod]
    public async Task NegativeAcknowledgeAsync_Requeues_Message_With_Incremented_DeliveryCount()
    {
        IQueueProvider queue = BuildQueueProvider();
        string queueName = $"inkwell-test-{Guid.NewGuid():N}";
        CancellationTokenSource cts = new CancellationTokenSource(TimeSpan.FromSeconds(10));
    ...
    private static IQueueProvider BuildQueueProvider()
    {
        ...
        builder.UseRedisQueue(s_container!.GetConnectionString());
