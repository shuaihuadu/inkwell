...
{
    private static QdrantContainer? s_container;

    [ClassInitialize]
    public static async Task ClassInitializeAsync(TestContext _)
    {
        s_container = new QdrantBuilder("qdrant/qdrant:latest").Build();
    ...
    [TestMethod]
    public async Task UpsertAsync_Then_GetAsync_Roundtrips_Record()
    {
    ...
    private static Microsoft.Extensions.VectorData.VectorStore BuildVectorStore()
    {
        ...
        Uri grpcEndpoint = new Uri(s_container!.GetGrpcConnectionString());
