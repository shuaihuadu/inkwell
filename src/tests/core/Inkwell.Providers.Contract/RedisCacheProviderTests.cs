...
{
    private static RedisContainer? s_container;

    [ClassInitialize]
    public static async Task ClassInitializeAsync(TestContext _)
    {
        s_container = new RedisBuilder("redis:8-alpine").Build();
    ...
    [TestMethod]
    public async Task GetAsync_On_Missing_Key_Returns_Default()
    {
        ICacheProvider cache = BuildCacheProvider();
    ...
    [TestMethod]
    public async Task IncrementAsync_Accumulates_Delta()
    {
        ICacheProvider cache = BuildCacheProvider();
    ...
    [TestMethod]
    public async Task TryAcquireLockAsync_Blocks_Second_Acquirer_Until_Released()
    {
        ICacheProvider cache = BuildCacheProvider();
    ...
    private static ICacheProvider BuildCacheProvider()
    {
        ...
        builder.UseRedisCache(s_container!.GetConnectionString());
